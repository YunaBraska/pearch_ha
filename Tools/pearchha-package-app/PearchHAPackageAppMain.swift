...
@main
struct PearchHAPackageAppCommand {
    private static let defaultAppIconURL = URL(fileURLWithPath: "Xcode/PearchHA/PearchHA.icns", isDirectory: false)

    static func main() {
        do {
            try run()
        } catch {
            FileHandle.standardError.write(Data("pearchha-package-app: \(error)\n".utf8))
            Foundation.exit(1)
        }
    }
    ...
    private static func run() throws {
        ...
        let manifest = try PearchHAAppBundleManifest(
            bundleIdentifier: options.value(for: "--bundle-id") ?? "dev.pearchha.app",
            ...
            callbackURLScheme: options.value(for: "--callback-scheme") ?? "pearchha",
        ...
        let result = try PearchHAAppBundleBuilder().build(
            PearchHAAppBundleBuildConfiguration(
                executableURL: URL(fileURLWithPath: options.value(for: "--executable") ?? ".build/debug/PearchHA"),
                outputURL: URL(fileURLWithPath: options.value(for: "--output") ?? ".build/PearchHA.app"),
                manifest: manifest,
                iconURL: iconURL,
                replaceExisting: options.flags.contains("--replace")
            )
        ...
        if options.flags.contains("--verify-launch-services") {
            let verification = try PearchHALaunchServicesVerifier().verify(
        ...
        if let signingIdentity {
            ...
            let signing = try PearchHACodeSigner(
                codesignURL: URL(fileURLWithPath: options.value(for: "--codesign") ?? PearchHACodeSigner.defaultCodesignURL.path)
            ).sign(
        ...
        if signingIdentity != nil || options.flags.contains("--verify-signature") {
            let verification = try PearchHACodeSignatureVerifier(
                codesignURL: URL(fileURLWithPath: options.value(for: "--codesign") ?? PearchHACodeSigner.defaultCodesignURL.path)
            ).verify(
        ...
        let hdiutilURL = URL(fileURLWithPath: options.value(for: "--hdiutil") ?? PearchHADMGBuilder.defaultHdiutilURL.path)
        let signatureVerified = signingIdentity != nil || options.flags.contains("--verify-signature")
        if options.flags.contains("--package-dmg") {
            let dmg = try PearchHADMGBuilder(hdiutilURL: hdiutilURL).build(
        ...
        if options.flags.contains("--verify-dmg") {
            let verification = try PearchHADMGVerifier(hdiutilURL: hdiutilURL).verify(
                PearchHADMGVerificationConfiguration(dmgURL: dmgOutputURL)
            )
            print("DMG verified: \(verification.dmgURL.path)")
        }
        if options.flags.contains("--verify-dmg-contents") {
            let verification = try PearchHADMGContentVerifier(hdiutilURL: hdiutilURL).verify(
        ...
        if let notaryProfile = options.value(for: "--notary-profile") {
            ...
            let submission = try PearchHANotarySubmitter(
                xcrunURL: URL(fileURLWithPath: options.value(for: "--xcrun") ?? PearchHANotarySubmitter.defaultXcrunURL.path)
            ).submit(
        ...
        if let releaseManifestPath = options.value(for: "--release-manifest") {
            ...
            let verification = try PearchHAReleaseEvidenceVerifier().verify(
    ...
    pearchha-package-app

    Usage:
      pearchha-package-app help
      pearchha-package-app --write-oauth-site PATH [--oauth-env PATH] [--replace]
      pearchha-package-app --verify-oauth-site PATH [--oauth-env PATH]
      pearchha-package-app --verify-published-oauth-site [--oauth-env PATH]
      pearchha-package-app [--executable PATH] [--output PATH] [--callback-scheme SCHEME] [--replace] [--verify-launch-services] [--sign-identity IDENTITY|--sign-ad-hoc] [--verify-signature] [--package-dmg] [--verify-dmg] [--verify-dmg-contents] [--release-manifest PATH] [--verify-release-manifest PATH] [--bundle-release-evidence PATH] [--oauth-site PATH] [--require-screenshot NAME] [--notary-profile PROFILE]
    ...
    Options:
      ...
      --oauth-env PATH              Env file path used for OAuth site generation or verification. Default: .env.local or PEARCHHA_ENV_FILE.
      --executable PATH             SwiftPM-built PearchHA executable. Default: .build/debug/PearchHA
      --output PATH                 Output .app bundle. Default: .build/PearchHA.app
      --dmg-output PATH             Output DMG path. Default: app output path with .dmg extension
      --dmg-volume-name NAME        DMG volume name. Default: PearchHA
      ...
      --callback-scheme SCHEME      OAuth callback URL scheme. Default: pearchha
      --bundle-id ID                Bundle identifier. Default: dev.pearchha.app
      ...
      --verify-release-manifest PATH
                                   ...
                                   PearchHA's canonical smoke screenshot set is required by default.
    ...
    private static func releaseRequiredScreenshotNames(_ additionalNames: [String]) -> [String] {
        var seen = Set(PearchHAReleaseEvidenceScreenshots.requiredNames)
        var names = PearchHAReleaseEvidenceScreenshots.requiredNames
    ...
    private static func releasePreflight(options: CommandOptions, signingIdentity: String?) throws {
        ...
        let report = try PearchHAReleasePreflightChecker(
            securityURL: URL(fileURLWithPath: options.value(for: "--security") ?? PearchHAReleasePreflightChecker.defaultSecurityURL.path),
            xcrunURL: URL(fileURLWithPath: options.value(for: "--xcrun") ?? PearchHANotarySubmitter.defaultXcrunURL.path)
        ).check(configuration)
    ...
    private static func writeOAuthSite(path: String, options: CommandOptions) throws {
        ...
        let result = try PearchHAOAuthClientWebsiteBuilder().build(
    ...
    private static func verifyPublishedOAuthSite(options: CommandOptions) throws {
        ...
        guard let siteURL = URL(string: manifest.clientID) else {
            throw PearchHAOAuthClientWebsiteError.invalidClientID(manifest.clientID)
        }
        ...
        let task = URLSession.shared.dataTask(with: siteURL) { data, response, error in
            ...
            guard let data,
                ...
                resultBox.store(.failure(PearchHAOAuthClientWebsiteError.publishedSiteUnreachable(manifest.clientID)))
                return
            }
        ...
        do {
            payload = try resultBox.load()?.get() ?? {
                throw PearchHAOAuthClientWebsiteError.publishedSiteUnreachable(manifest.clientID)
            }()
        } catch {
            if let knownError = error as? PearchHAOAuthClientWebsiteError {
                throw knownError
            }
            throw PearchHAOAuthClientWebsiteError.publishedSiteUnreachable(manifest.clientID)
        }
        guard (200...299).contains(payload.1.statusCode) else {
            throw PearchHAOAuthClientWebsiteError.publishedSiteHTTPFailure(manifest.clientID, payload.1.statusCode)
        }
        let verification = try PearchHAOAuthClientWebsiteVerifier().verify(
            data: payload.0,
            siteURL: siteURL,
    ...
    private static func oauthSiteManifest(options: CommandOptions) throws -> PearchHAOAuthClientWebsiteManifest {
        ...
        return try PearchHAOAuthClientWebsiteManifest(
    ...
    private static func parseOptions(arguments: [String]) throws -> CommandOptions {
        do {
            let parsed = try PearchHACommandLineOptions(
                arguments: arguments,
                valueOptions: [
                    "--write-oauth-site",
                    "--verify-oauth-site",
...
private struct CommandOptions {
    ...
    private let parsed: PearchHACommandLineOptions

    init(parsed: PearchHACommandLineOptions) {
