...
import PearchHARepoAudit
import PearchHASupport
import PearchHAUI

@main
struct PearchHASmoke {
    static func main() async {
        do {
            try await run(arguments: Array(CommandLine.arguments.dropFirst()))
        } catch {
            fputs("pearchha-smoke: \(error)\n", stderr)
            Foundation.exit(1)
        }
    }
    ...
    private static func run(arguments: [String]) async throws {
        ...
        if options.repeatCount > 1 {
            ...
            print("PearchHA smoke verification passed.")
            return
        }
    ...
    private static func runOnce(options: SmokeOptions) async throws {
        try expect(PearchHASupport.module.name == "PearchHASupport", "support module is named")
        try expect(PearchHACore.module.name == "PearchHACore", "core module is named")
        try expect(PlannedHAClient().describe().name == "PearchHAClient", "client module is named")
        try expect(PlannedConfigStore().describe().name == "PearchHAPersistence", "persistence module is named")
        try expect(PearchHAUI.module.name == "PearchHAUI", "UI module is named")
        ...
        try expect(location.applicationSupportDirectoryName == "PearchHA", "config location uses app name")
    ...
    private static func runRepeatedProcesses(arguments: [String], repeatCount: Int) throws {
        ...
        for run in 1...repeatCount {
            ...
            print("PearchHA smoke verification passed (\(run)/\(repeatCount)).")
        }
    }
    ...
    private static func verifyRateLimiter() throws {
        let start = PearchInstant(nanosecondsSinceStart: 0)
    ...
    private static func verifyCoverageGate() throws {
        let directory = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("PearchHACoverageSmoke-\(UUID().uuidString)", isDirectory: true)
        ...
        try Data(
            """
            {"data":[{"files":[{"filename":"Sources/PearchHACore/Foo.swift","summary":{"lines":{"count":10,"covered":10}}}]}]}
            """.utf8
        ...
        try expect(
            missingBranch.error.contains("branch coverage for Sources/PearchHACore"),
        ...
        try expect(
            zeroBranch.output.contains("PearchHACore branch: 100.00% >= 100.00% (0/0)"),
    ...
    private static func verifyXcodeDoctor() throws {
        ...
        let diagnostic = try JSONDecoder().decode(
            PearchHAXcodePreflightDiagnostic.self,
    ...
    private static func verifyRepositoryAuditGate() throws {
        ...
        try verifyRepositoryAuditRejects(
            relativePath: "PearchHA.xcodeproj/project.pbxproj",
            contents: "repositoryURL = https://github.com/" + "get" + "sentry" + "/" + "sentry" + "-cocoa;\n",
            expectedError: "PearchHA.xcodeproj/project.pbxproj: telemetry dependency or collection endpoint must not be present by default",
            addToIndex: true,
    ...
    private static func createRepositoryAuditSmokeRepository(
        ...
        let directory = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("PearchHARepoAuditSmoke-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
    ...
    private static func runRepositoryAudit(_ arguments: [String]) -> RepoAuditSmokeResult {
        var output: [String] = []
        var errors: [String] = []
        let exitCode = PearchHARepoAuditCommand.run(
            arguments: arguments,
            standardOutput: { output.append($0) },
            standardError: { errors.append($0) }
        )
    ...
    private static func verifyTestClock() async throws {
        ...
        try await expectAsync(await clock.now() == PearchInstant(nanosecondsSinceStart: 0), "test clock starts at zero")
        _ = await clock.advance(by: .seconds(3))
        try await expectAsync(await clock.now() == PearchInstant(nanosecondsSinceStart: 3_000_000_000), "test clock advances")

        let wakeDeadline = PearchInstant(nanosecondsSinceStart: 5_000_000_000)
        let sleeper = Task {
        ...
        let cancelled = Task {
            try await clock.sleep(until: PearchInstant(nanosecondsSinceStart: 10_000_000_000))
        }
    ...
    private static func verifyMirrorAndFakeHA() async throws {
        ...
        let fixtureDirectory = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("pearchha-smoke-fixtures-\(UUID().uuidString)", isDirectory: true)
        defer {
    ...
    private static func verifyHAMirrorServeCLI() async throws {
        let fixtureDirectory = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("pearchha-smoke-serve-\(UUID().uuidString)", isDirectory: true)
        defer {
            try? FileManager.default.removeItem(at: fixtureDirectory)
        }
    ...
    private static func verifyHAMirrorCaptureCLI() async throws {
        ...
        let envURL = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("pearchha-smoke-hamirror-\(UUID().uuidString).env", isDirectory: false)
    ...
    private static func verifyHAMirrorDoctorCLIUsesExportedOverrides() async throws {
        ...
        let envURL = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("pearchha-smoke-hamirror-doctor-\(UUID().uuidString).env", isDirectory: false)
        defer {
            try? FileManager.default.removeItem(at: envURL)
        }
        ...
        let output = try runHAMirrorProcess(
            ...
            environment: [
                HAMirrorEnvironment.environmentFileEnvironmentKey: envURL.path,
                HAMirrorEnvironment.tokenEnvironmentKey: "fake-token",
                HAMirrorEnvironment.oauthClientIDEnvironmentKey: "https://pearchha.dev/app",
                HAMirrorEnvironment.oauthRedirectURIEnvironmentKey: "pearchha://auth"
            ]
        )
    ...
    private static func verifyHAMirrorDoctorProbeReportsLiveGuidance() async throws {
        ...
        let envURL = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("pearchha-smoke-hamirror-doctor-probe-\(UUID().uuidString).env", isDirectory: false)
        defer {
            try? FileManager.default.removeItem(at: envURL)
        }
    ...
    private static func verifyHAMirrorOAuthCheckReportsMissingConfigurationGuidance() async throws {
        ...
        let envURL = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("pearchha-smoke-hamirror-oauth-check-\(UUID().uuidString).env", isDirectory: false)
        defer {
            try? FileManager.default.removeItem(at: envURL)
        }
        ...
        try expect(
            output.text.contains("PEARCHHA_OAUTH_CLIENT_ID and PEARCHHA_OAUTH_REDIRECT_URI"),
    ...
    private static func launchHAMirrorServe(fixtures: URL, pathPrefix: String) throws -> ServeProcessHandle {
        ...
        let outputURL = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("pearchha-smoke-serve-log-\(UUID().uuidString).txt", isDirectory: false)
        ...
        let oauthTransport = SmokeHARESTTransport(
            responses: [
                HARESTResponse(
                    ...
                    body: Data(#"<link rel="redirect_uri" href="pearchha://auth">"#.utf8)
                )
            ]
        )
    ...
    private static func verifyHAWebSocketContract() async throws {
        ...
        let olderServerFallback = try FakeHAWebSocketServer(
            fixtures: fallbackHistoryFixtures,
            mode: .unsupportedCommands(["recorder/statistics_during_period"])
        )
        olderServerFallback.start()
        defer {
        ...
        try expect(
            olderServerFallbackResult == .success(expectedFallbackHistory),
            "HA client falls back to REST history when recorder statistics is older-HA unsupported"
        )
        try await spinUntil("FakeHA journals older-HA recorder statistics fallback") {
            let paths = await olderServerFallback.journal.snapshot().map(\.path)
            return paths.contains("/api/websocket/recorder/statistics_during_period")
    ...
    private static func verifyDiscoveryAndPersistence() async throws {
        ...
        let configURL = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("pearchha-smoke-config-\(UUID().uuidString)", isDirectory: true)
        ...
        let configuration = PearchHAConfiguration(
        ...
        let secretStore = KeychainSecretStore(service: "dev.pearchha.smoke.\(UUID().uuidString)")
        defer {
    ...
    private static func verifyPanelModelAgainstFakeHA() async throws {
        ...
        let empty = PearchHAPanelModel { _ in
            .success(rooms: [])
        }
        ...
        let failing = PearchHAPanelModel()
        ...
        let secret = PearchHAPanelModel { form in
        ...
        let sequenced = PearchHAPanelModel { form in
        ...
        let cleared = PearchHAPanelModel { form in
        ...
        let invalidFallback = PearchHAPanelModel()
        ...
        let fallback = PearchHAPanelModel { form in
        ...
        let certificate = PearchHAPanelModel { form in
        ...
        let historyClock = TestPearchClock()
        ...
        let historyPanel = PearchHAPanelModel(
        ...
        historyPanel.dismissHistoryPopover()
        try expect(
        ...
        let sparkline = PearchHAHistorySparklineGeometry(
            series: HistorySeries(
                entityID: "sensor.office_temperature",
                range: .hour,
                samples: [
        ...
        let mixedHistorySummary = PearchHAHistoryContentSummary(
            series: HistorySeries(
                entityID: "sensor.office_temperature",
                range: .hour,
                samples: [
        ...
        try expect(
            mixedHistorySummary == .statistics(
                PearchHAHistoryStatistics(current: 30, minimum: 10, average: 20, maximum: 30)
            ),
            "panel history summary uses latest chronological numeric sample"
        )
        let unsortedSparkline = PearchHAHistorySparklineGeometry(
            series: HistorySeries(
                entityID: "sensor.office_temperature",
                range: .hour,
                samples: [
        ...
        let flatSparkline = PearchHAHistorySparklineGeometry(
            series: HistorySeries(
                entityID: "sensor.office_temperature",
                range: .hour,
                samples: [
        ...
        let disconnectedHistory = PearchHAPanelModel()
        ...
        let gate = Gate()
        let cancellable = PearchHAPanelModel { _ in
            await gate.wait()
            return .success(
                rooms: [
                    Room(
        ...
        let staleSnapshot = PearchHAPanelSnapshot(
        ...
        let failingRefresh = PearchHAPanelModel { _ in
        ...
        let selectable = PearchHAPanelModel(
            connector: { _ in
                .success(
                    rooms: [
                        Room(
        ...
        let controls = PearchHAPanelModel(
        ...
        let protectedControls = PearchHAPanelModel(
            connector: { _ in .success(rooms: controlRooms) },
        ...
        let failingControl = PearchHAPanelModel(
            connector: { _ in .success(rooms: controlRooms) },
        ...
        let reorder = PearchHAPanelModel(
            connector: { _ in
                .success(
                    rooms: [
                        Room(
        ...
        let vanished = PearchHAPanelModel(
            connector: { _ in
                .success(
                    rooms: [
                        Room(
    ...
    private static func verifyAppShellPanelFactory() throws {
        ...
        let settingsWindow = PearchHAApplication.makeSettingsWindow(
    ...
    private static func verifyHistoryPopoverPanelFactory() throws {
        ...
        let view = PearchHAHistoryPopoverContent(
    ...
    private static func verifyIdleCPUAtRest() async throws {
        ...
        let serviceMetadataProvider = fakeHAServiceMetadataProvider(client: client)
        let configURL = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("pearchha-smoke-idle-cpu-\(UUID().uuidString)", isDirectory: true)
            .appendingPathComponent("config.json")
        defer {
            try? FileManager.default.removeItem(at: configURL.deletingLastPathComponent())
        }
    ...
    private static func fakeHAConnector(client: HomeAssistantClient) -> PearchHAPanelModel.Connector {
        { form in
            guard let primaryURL = form.primaryURL() else {
                return .failure(.protocolError("invalid Home Assistant URL"))
            }
    ...
    private static func fakeHAServiceMetadataProvider(client: HomeAssistantClient) -> PearchHAPanelModel.ServiceMetadataProvider {
        { form in
            guard let primaryURL = form.primaryURL() else {
                return .unavailable("invalid Home Assistant URL")
            }
    ...
    private static func runApplicationLifecycleSoakIteration(
        ...
        serviceMetadataProvider: @escaping PearchHAPanelModel.ServiceMetadataProvider
    ) async throws {
        let configURL = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("pearchha-smoke-lifecycle-soak-\(UUID().uuidString)", isDirectory: true)
            .appendingPathComponent("config.json")
        defer {
            try? FileManager.default.removeItem(at: configURL.deletingLastPathComponent())
        }
    ...
    private static func verifyPanelSnapshotRendering(options: SmokeOptions) async throws {
        ...
        if options.updatesReviewBaseline {
            ...
            if storedBaseline.renderEnvironment == reviewBaseline.renderEnvironment {
                ...
                print(
                    "pearchha-smoke: review baseline recorded on \(storedBaseline.renderEnvironment ?? "an unrecorded environment"), "
    ...
    @MainActor
    private static func panelSnapshotModel(for variant: SmokePanelSnapshotVariant) async throws -> PearchHAPanelModel {
        ...
        if variant.startsOAuthSignInForSnapshot {
            ...
            for _ in 0..<100 {
                if model.oauthSignInState == PearchHAOAuthSignInState.signingIn {
                    break
                }
                await Task.yield()
            }
            try expect(
                model.oauthSignInState == PearchHAOAuthSignInState.signingIn,
    ...
    private static func warmedInlineHistoryModel(
    ...
    ) async throws -> PearchHAPanelModel {
        let snapshot = variant.snapshot
        let clock = TestPearchClock()
        ...
        let model = PearchHAPanelModel(
            ...
            bulkSyncConfiguration: PearchHAHistoryBulkSyncConfiguration(settleDelay: settleDelay, coldRefreshDivisor: 1),
            selectionConfiguration: snapshot.selectionConfiguration,
            menuBarDisplayConfiguration: snapshot.menuBarDisplayConfiguration,
            customActionConfiguration: variant.customActionConfiguration
        )
    ...
    private static func renderPanelSnapshot(_ variant: SmokePanelSnapshotVariant) async throws -> SmokePanelRenderCapture {
        ...
        if variant == .historyLoadedLight || variant == .historyLoadedLightIncreasedContrast {
            ...
            let tab: PearchHASettingsView.Tab = variant == .settingsAboutUpdateLight ? .about : .entities
            view = AnyView(
                PearchHASettingsView(
                    model: model,
    ...
    private static func panelSnapshotExportDirectory() -> URL? {
        guard let value = ProcessInfo.processInfo.environment["PEARCHHA_SMOKE_SNAPSHOT_DIR"]?.trimmingCharacters(in: .whitespacesAndNewlines),
              !value.isEmpty
        else {
            return nil
        }
    ...
    private static func verifyOAuthConfigurationFromEnvironmentFile() throws {
        let directory = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("pearchha-smoke-env-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        defer {
            try? FileManager.default.removeItem(at: directory)
        }
        ...
        try expect(configuration?.clientID == "https://pearchha.dev/app", "OAuth config reads client ID from env file")
        try expect(configuration?.redirectURI == "pearchha://auth", "OAuth config reads redirect URI from env file")
        try expect(configuration?.callbackURLScheme == "pearchha", "OAuth config derives callback scheme")
    }

    private static func verifyOAuthClientWebsitePackaging() async throws {
        let directory = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("pearchha-smoke-oauth-site-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        defer {
            try? FileManager.default.removeItem(at: directory)
        }
        ...
        let executable = try toolExecutable(named: "pearchha-package-app")
        ...
        try expect(
            writeOutput.text.contains("OAuth client website verified: https://pearchha.dev/app -> pearchha://auth"),
            "OAuth client website packaging reports verification details"
        )
        let html = try String(contentsOf: outputURL, encoding: .utf8)
        try expect(html.contains(#"<link rel="redirect_uri" href="pearchha://auth">"#), "OAuth client website artifact declares redirect URI")
        ...
        try """
        PEARCHHA_OAUTH_CLIENT_ID=\(publishedURL.absoluteString)
        PEARCHHA_OAUTH_REDIRECT_URI=pearchha://auth
    ...
    private static func verifyAppBundlePackaging() throws {
        ...
        let callbackScheme = "pearchha-smoke-\(uniqueID)"
        let outputURL = buildDirectory.appendingPathComponent("pearchha-smoke-\(uniqueID).app", isDirectory: true)
        let dmgURL = buildDirectory.appendingPathComponent("pearchha-smoke-\(uniqueID).dmg", isDirectory: false)
        defer {
        ...
        let result = try PearchHAAppBundleBuilder().build(
            PearchHAAppBundleBuildConfiguration(
                ...
                manifest: try PearchHAAppBundleManifest(
                    bundleIdentifier: "dev.pearchha.smoke.\(uniqueID.replacingOccurrences(of: "-", with: ""))",
        ...
        let launchServices = try PearchHALaunchServicesVerifier().verify(
        ...
        let signing = try PearchHACodeSigner().sign(
        ...
        let signature = try PearchHACodeSignatureVerifier().verify(
            PearchHACodeSignatureVerificationConfiguration(appURL: result.appURL)
        )
        try expect(signature.appURL.path == result.appURL.path, "app bundle code signature verifies")
        let dmg = try PearchHADMGBuilder().build(
            PearchHADMGBuildConfiguration(
                appURL: result.appURL,
                outputURL: dmgURL,
                volumeName: "PearchHA Smoke"
            )
        )
        try expect(FileManager.default.fileExists(atPath: dmg.dmgURL.path), "DMG builder creates disk image")
        let dmgVerification = try PearchHADMGVerifier().verify(PearchHADMGVerificationConfiguration(dmgURL: dmg.dmgURL))
        try expect(dmgVerification.dmgURL.path == dmgURL.path, "DMG verifier validates disk image")
        let dmgContents = try PearchHADMGContentVerifier().verify(
            PearchHADMGContentVerificationConfiguration(
                dmgURL: dmg.dmgURL,
    ...
    private static func verifyApplicationLaunchWiring() async throws {
        ...
        guard let callbackURL = URL(string: "pearchha://auth?code=smoke-code&state=smoke-state") else {
        ...
        try expect(
            callbackEvent.disposition == PearchHAExternalURLDisposition.acceptedOAuthCallback,
        ...
        let configURL = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("pearchha-smoke-app-config-\(UUID().uuidString)", isDirectory: true)
            .appendingPathComponent("config.json")
        defer {
            try? FileManager.default.removeItem(at: configURL.deletingLastPathComponent())
        }
        ...
        let configured = PearchHAApplication(
            configStore: store,
            authSessionStore: PearchHAAuthSessionStore(secretStore: SmokeSecretStore())
        )
        ...
        let loadFailure = PearchHAApplication(
        ...
        let saveFailureStore = SmokeConfigStore(
            loadedConfiguration: PearchHAConfiguration(selectedEntityIDs: ["sensor.office_temperature"], isEntitySelectionExplicit: true),
            saveError: .writeFailed(URL(fileURLWithPath: "/tmp/pearchha-config.json"), message: "disk full")
        )
        let saveFailure = PearchHAApplication(
        ...
        let panelSaveFailure = PearchHAApplication(
            configStore: saveFailureStore,
            authSessionStore: PearchHAAuthSessionStore(secretStore: SmokeSecretStore())
        )
    ...
    private static func verifyMenuBarRenderingAndPromotion() async throws {
        ...
        let ringImage = PearchHAStatusItemGaugeImageRenderer().image(for: ring)
        ...
        let smokePalette = PearchHAStatusItemGaugePalette(
        ...
        let warningImage = PearchHAStatusItemGaugeImageRenderer(palette: smokePalette).image(for: warningGauge)
        try expect(
        ...
        let criticalRingImage = PearchHAStatusItemGaugeImageRenderer(palette: smokePalette).image(for: criticalRing)
        try expect(
        ...
        let configURL = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("pearchha-smoke-menu-bar-\(UUID().uuidString)", isDirectory: true)
            .appendingPathComponent("config.json")
        defer {
            try? FileManager.default.removeItem(at: configURL.deletingLastPathComponent())
        }
        ...
        let searchedMenuBar = PearchHAPanelModel(
            connector: { _ in .success(rooms: rooms) },
        ...
        let reconnecting = PearchHAPanelModel { _ in
        ...
        let failed = PearchHAPanelModel { _ in
...
private enum SmokePanelSnapshotVariant: String, CaseIterable {
    ...
    var accessibilityPreferences: PearchHAAccessibilityPreferences {
        PearchHAAccessibilityPreferences(
    ...
    var oauthSignInRunner: PearchHAPanelModel.OAuthSignInRunner {
        switch self {
        case .signingInLight:
            return { _ in
                do {
    ...
    var releaseUpdateChecker: PearchHAPanelModel.ReleaseUpdateChecker {
        switch self {
        case .settingsAboutUpdateLight:
            return { currentVersion in
                let releaseURL = URL(string: "https://github.com/YunaBraska/pearch_ha/releases/tag/2026.7.71500")!
                let downloadURL = URL(string: "https://github.com/YunaBraska/pearch_ha/releases/download/2026.7.71500/PearchHA-2026.7.71500.dmg")!
    ...
    var currentApplicationVersionProvider: @Sendable () -> String {
        ...
        case .settingsAboutUpdateLight:
            return { "1.0" }
        default:
            return { PearchHAApplicationVersionInfo.currentBundle().releaseVersion }
        }
    }
...
private struct SmokeOptions {
    ...
    Runs PearchHA's public smoke verification suite and optional screenshot baseline refresh.

    Usage:
      pearchha-smoke
      pearchha-smoke help
      PEARCHHA_SMOKE_SNAPSHOT_DIR=.build/pearchha-snapshots pearchha-smoke
      pearchha-smoke --repeat 5
      pearchha-smoke --update-review-baseline
      pearchha-smoke --review-baseline docs/release-review-baseline.json
    ...
    Environment:
      PEARCHHA_SMOKE_SNAPSHOT_DIR    Export screenshots under <dir>/current during the run.
    """
    ...
    init(arguments: [String]) throws {
        ...
        let showsHelp: Bool
        do {
            let options = try PearchHACommandLineOptions(
                arguments: arguments,
...
private enum CommandLineOptions {
    static func parseErrorDescription(_ error: PearchHACommandLineParseError) -> String {
        switch error {
...
@MainActor
final class CountingStatusItemGaugeImageRenderer: PearchHAStatusItemGaugeImageRendering {
