...
public struct HAMirrorEnvironment: Equatable, Sendable {
    ...
    public static let primaryURLEnvironmentKey = "PERCHHA_HA_URL"
    public static let fallbackURLEnvironmentKey = "PERCHHA_HA_FALLBACK_URL"
    public static let tokenEnvironmentKey = "PERCHHA_HA_TOKEN"
    public static let userEnvironmentKey = "PERCHHA_HA_USER"
    public static let passwordEnvironmentKey = "PERCHHA_HA_PASSWORD"
    public static let oauthClientIDEnvironmentKey = "PERCHHA_OAUTH_CLIENT_ID"
    public static let oauthRedirectURIEnvironmentKey = "PERCHHA_OAUTH_REDIRECT_URI"
    ...
    public static func fromEnvironment(
        ...
        return HAMirrorEnvironment(
            ...
            oauthClientID: environmentValue(for: oauthClientIDEnvironmentKey, environment: environment)
                ?? blankToNil(fileValues["perchha_oauth_client_id"]),
            oauthRedirectURI: environmentValue(for: oauthRedirectURIEnvironmentKey, environment: environment)
                ?? blankToNil(fileValues["perchha_oauth_redirect_uri"])
        )
    }
...
public struct HAMirrorEnvironmentReadinessReport: Equatable, Sendable {
    ...
    public func nextSteps(envPath: String) -> [String] {
        ...
        if !hasToken {
            result.append(
                "Set token= in \(envPath) or export PERCHHA_HA_TOKEN to a Home Assistant bearer access token before mirror capture. A long-lived access token works, or use the native OAuth sign-in flow and copy the resulting access token."
            )
        }
        if (hasUser || hasPassword) && !hasToken {
            result.append(
                "user/password alone cannot authenticate the REST or WebSocket APIs. Keep them only for browser sign-in or manual work; mirror capture still needs token= in \(envPath) or PERCHHA_HA_TOKEN in the process environment."
            )
        }
        if !hasOAuthClientID && !hasOAuthRedirectURI {
            result.append(
                "If you want native OAuth sign-in instead of a long-lived token, set both PERCHHA_OAUTH_CLIENT_ID and PERCHHA_OAUTH_REDIRECT_URI in \(envPath), or export both values, then run hamirror oauth-check --env \(quotedEnvPath)."
            )
        } else if hasOAuthClientID != hasOAuthRedirectURI {
            result.append(
                "Set both PERCHHA_OAUTH_CLIENT_ID and PERCHHA_OAUTH_REDIRECT_URI in \(envPath), or export both values, then rerun hamirror oauth-check --env \(quotedEnvPath)."
            )
        }
        return result
    }
    ...
    public func oauthCheckBlockingMessages(envPath: String) -> [String] {
        var result: [String] = []
        if !hasOAuthClientID && !hasOAuthRedirectURI {
            result.append("OAuth client website check requires PERCHHA_OAUTH_CLIENT_ID and PERCHHA_OAUTH_REDIRECT_URI")
        } else if hasOAuthClientID != hasOAuthRedirectURI {
        ...
        result.append(
            contentsOf: nextSteps(envPath: envPath).filter {
                $0.contains("hamirror oauth-check") || $0.contains("PERCHHA_OAUTH_CLIENT_ID")
            }
        )
        return result
    }
...
public struct HAOAuthClientWebsiteEnvironment: Equatable, Sendable {
    ...
    public static func fromEnvironment(
        ...
        return try HAOAuthClientWebsiteEnvironment(
            clientID: environmentClientID ?? fileValues["perchha_oauth_client_id"] ?? "",
            redirectURI: environmentRedirectURI ?? fileValues["perchha_oauth_redirect_uri"] ?? ""
        )
    }
