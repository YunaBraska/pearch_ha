...
public struct TokenBucketRateLimiter: Sendable {
    ...
    private var lastRefill: PearchInstant

    public init(capacity: Double, refillPerSecond: Double, now: PearchInstant) {
    ...
    public mutating func tryAcquire(tokens: Double = 1, at now: PearchInstant) -> Bool {
    ...
    public mutating func available(at now: PearchInstant) -> Double {
        refill(at: now)
        return availableTokens
    }

    public mutating func nextAvailability(for tokens: Double = 1, at now: PearchInstant) -> RateLimitAvailability {
        precondition(tokens > 0, "tokens must be positive")
        ...
        return .available(now.advanced(by: PearchDuration(nanoseconds: Int64(seconds * 1_000_000_000))))
    }

    private mutating func refill(at now: PearchInstant) {
