...
public struct PearchHAMenuBarPresentation: Equatable, Sendable {
    public static let fallback = PearchHAMenuBarPresentation(
        entityID: nil,
        title: "PearchHA",
...
public struct PearchHAMenuBarPresenter: Sendable {
    ...
    public func presentation(
        ...
        panelSnapshot: PearchHAPanelSnapshot,
        locale: Locale = .current
    ) -> PearchHAMenuBarPresentation {
        presentations(
    ...
    ///   a single-element array holding ``PearchHAMenuBarPresentation/fallback``
    ...
    public func presentations(
        ...
        return entities.map { entity in
            ...
            let iconSymbolName: String? = rendered.gauge == nil && itemConfiguration.showsEntityIcon
                ? (itemConfiguration.customIconName ?? pearchHAEntityIconName(for: entity))
                : nil
            ...
            let stackedLabel: String? = itemConfiguration.showsLabel && !trimmedName.isEmpty
                ? trimmedName
                : nil
            return PearchHAMenuBarPresentation(
                entityID: entity.id,
