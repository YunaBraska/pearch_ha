...
private final class PearchHAFirstMouseHostingView<Content: View>: NSHostingView<Content> {
    override func acceptsFirstMouse(for event: NSEvent?) -> Bool {
        true
    }
}

private final class PearchHAFirstMouseHostingController<Content: View>: NSHostingController<Content> {
    override func loadView() {
        view = PearchHAFirstMouseHostingView(rootView: rootView)
    }
}

public final class PearchHAStatusPanel: NSPanel {
    /// Invoked when the user presses Cmd+, inside the panel, routing to the
    /// app shell's open-settings path. Set by the owning ``PearchHAApplication``.
...
private final class PearchHASettingsWindow: NSWindow {
...
public struct PearchHAMenuBarStatusItemSnapshot: Equatable, Sendable {
...
public struct PearchHAApplicationSnapshot: Equatable, Sendable {
    ...
    public let menuBarItems: [PearchHAMenuBarStatusItemSnapshot]
    ...
    public let configurationPersistenceState: PearchHAConfigurationPersistenceState
    public let historyState: PearchHAHistoryPanelState
    public let historyPresentationEntityID: EntityID?
    public let controlActionState: PearchHAControlActionState
    public let lastExternalURLEvent: PearchHAExternalURLEvent?
    public let connectionState: ConnectionState
    public let connectionForm: PearchHAConnectionForm
    public let hasTokenInput: Bool
}

public enum PearchHAConfigurationPersistenceState: Equatable, Sendable {
    case ready
    case unavailable
...
public struct PearchHAExternalURLEvent: Equatable, Sendable {
    ...
    public let disposition: PearchHAExternalURLDisposition
}

public enum PearchHAExternalURLDisposition: Equatable, Sendable {
...
public protocol PearchHAAuthSessionStorage: Sendable {
    @discardableResult func save(_ session: PearchHAAuthSession) throws -> PearchHAAuthSessionWriteResult
    func load() throws -> PearchHAAuthSession
    ...
    @discardableResult func clear() throws -> PearchHAAuthSessionClearResult
}

extension PearchHAAuthSessionStore: PearchHAAuthSessionStorage {}

public protocol PearchHARefreshingHomeAssistantClient: Sendable {
...
public protocol PearchHAServerTrustRefreshingHomeAssistantClient: PearchHARefreshingHomeAssistantClient {
...
extension HomeAssistantClient: PearchHARefreshingHomeAssistantClient {}
extension HomeAssistantClient: PearchHAServerTrustRefreshingHomeAssistantClient {}

public struct PearchHAAuthorizedHomeAssistantGateway: Sendable {
    private let client: any PearchHARefreshingHomeAssistantClient
    private let authSessionStore: any PearchHAAuthSessionStorage

    public init(
        client: any PearchHARefreshingHomeAssistantClient = HomeAssistantClient(),
        authSessionStore: any PearchHAAuthSessionStorage = PearchHAAuthSessionStore()
    ) {
        self.client = client
        self.authSessionStore = authSessionStore
    }

    public func connect(form: PearchHAConnectionForm) async -> PearchHAConnectionAttemptResult {
    ...
    public func services(form: PearchHAConnectionForm) async -> PearchHAServiceMetadataProviderResult {
        guard let primaryURL = form.primaryURL() else {
            return .unavailable("invalid Home Assistant URL")
        }
    ...
    public func action(form: PearchHAConnectionForm, action: ActionSpec) async -> PearchHAActionResult {
        guard let primaryURL = form.primaryURL() else {
    ...
    private func resolveInput(form: PearchHAConnectionForm, primaryURL: URL) -> HAClientResult<PearchHAAuthorizedInput> {
        let urls = form.urls()
        let endpoint = HAEndpoint(urls: urls.isEmpty ? [primaryURL] : urls)
        let serverTrustPolicy = PearchHAServerTrustPolicyResolver.policy(for: form)
        if !form.trimmedToken.isEmpty {
            return .success(
                PearchHAAuthorizedInput(
    ...
    private func resolveAuthenticationFailure<Value: Sendable>(
        ...
        authorizedInput: PearchHAAuthorizedInput,
    ...
    private func refreshAccessToken(
        ...
        if let trustPolicyClient = client as? any PearchHAServerTrustRefreshingHomeAssistantClient {
    ...
    private func persistRefreshedToken(_ token: HAOAuthToken, previousSession: PearchHAAuthSession) throws {
...
public struct PearchHAOAuthApplicationConfiguration: Equatable, Sendable {
    public static let clientIDEnvironmentKey = "PEARCHHA_OAUTH_CLIENT_ID"
    public static let redirectURIEnvironmentKey = "PEARCHHA_OAUTH_REDIRECT_URI"
    ...
    public init(clientID: String, redirectURI: String) throws {
        ...
        guard !normalizedClientID.isEmpty else {
            throw PearchHAOAuthSignInFailure.configuration("OAuth client ID is not configured")
        }
        guard let clientIDURL = URL(string: normalizedClientID),
            ...
            throw PearchHAOAuthSignInFailure.configuration("OAuth client ID must be an application website URL")
        }
        guard let redirectURL = URL(string: normalizedRedirectURI),
              let scheme = redirectURL.scheme,
              !scheme.isEmpty
        else {
            throw PearchHAOAuthSignInFailure.configuration("OAuth redirect URI is invalid")
        }
    ...
    public static func fromEnvironment(_ environment: [String: String] = ProcessInfo.processInfo.environment) throws -> PearchHAOAuthApplicationConfiguration? {
        ...
        if environmentClientID != nil, environmentRedirectURI != nil {
            return try PearchHAOAuthApplicationConfiguration(
    ...
    public static func parseEnvironmentFile(_ contents: String) throws -> [String: String] {
        ...
        for (index, rawLine) in contents.split(separator: "\n", omittingEmptySubsequences: false).enumerated() {
            ...
            guard let separator = line.firstIndex(of: "=") else {
                throw PearchHAOAuthEnvironmentFileError.invalidLine(index + 1)
            }
    ...
    private static func environmentFileValues(environment: [String: String]) throws -> [String: String] {
        ...
        guard FileManager.default.fileExists(atPath: path) else {
            if explicitPath {
                throw PearchHAOAuthEnvironmentFileError.missingFile(path)
            }
            return [:]
        }
        do {
        ...
        } catch let error as PearchHAOAuthEnvironmentFileError {
            throw error
        } catch {
            throw PearchHAOAuthEnvironmentFileError.unreadableFile(path)
        }
    }
...
public enum PearchHAOAuthEnvironmentFileError: Error, Equatable, CustomStringConvertible, Sendable {
...
public enum PearchHAOAuthPresentationResult: Equatable, Sendable {
    case callback(URL)
    case cancelled
    case failed(String)
}

public protocol PearchHAOAuthAuthorizationPresenter: Sendable {
...
public enum PearchHAOAuthSignInFailure: Error, Equatable, CustomStringConvertible, Sendable {
...
public enum PearchHAServerTrustPolicyResolver {
    ...
    public static func policy(for form: PearchHAConnectionForm) -> HAServerTrustPolicy {
...
public struct PearchHAOAuthSignInCoordinator: Sendable {
    private let configuration: PearchHAOAuthApplicationConfiguration?
    private let client: HomeAssistantClient
    private let authSessionStore: any PearchHAAuthSessionStorage
    private let presenter: any PearchHAOAuthAuthorizationPresenter
    private let stateGenerator: @Sendable () -> String

    @MainActor
    public init(
        configuration: PearchHAOAuthApplicationConfiguration?,
        client: HomeAssistantClient = HomeAssistantClient(),
        authSessionStore: any PearchHAAuthSessionStorage = PearchHAAuthSessionStore(),
        presenter: any PearchHAOAuthAuthorizationPresenter = PearchHAASWebAuthenticationSessionPresenter(),
        stateGenerator: @escaping @Sendable () -> String = PearchHAOAuthSignInCoordinator.secureState
    ) {
    ...
    @MainActor
    public func signIn(form: PearchHAConnectionForm) async -> PearchHAOAuthSignInResult {
        do {
            try await performSignIn(form: form)
            return .success
        } catch let failure as PearchHAOAuthSignInFailure {
    ...
    @MainActor
    private func performSignIn(form: PearchHAConnectionForm) async throws {
        guard let configuration else {
            throw PearchHAOAuthSignInFailure.configuration("OAuth sign-in is not configured")
        }
        guard let primaryURL = form.primaryURL() else {
            throw PearchHAOAuthSignInFailure.callback("invalid Home Assistant URL")
        }
        ...
        case let .success(url):
            authorizationURL = url
        case let .failure(failure):
            throw PearchHAOAuthSignInFailure.authorizationURL(failure.description)
        }
        ...
        case let .callback(url):
            callbackURL = url
        case .cancelled:
            throw PearchHAOAuthSignInFailure.cancelled
        case let .failed(message):
            throw PearchHAOAuthSignInFailure.presentation(message)
        }
        ...
        let exchange = await client.exchangeAuthorizationCode(
            ...
            code: callback.code,
            clientID: configuration.clientID,
            serverTrustPolicy: PearchHAServerTrustPolicyResolver.policy(for: form)
        )
        ...
        case let .success(value):
            token = value
        case let .failure(failure):
            throw PearchHAOAuthSignInFailure.exchange(failure.description)
        }
        guard let refreshToken = token.refreshToken else {
            throw PearchHAOAuthSignInFailure.exchange("refresh_token is required")
        }
        ...
        do {
            ...
            throw PearchHAOAuthSignInFailure.storage(String(describing: error))
        }
    }
    ...
    private func parseCallback(
        ...
        guard let expectedRedirectComponents = URLComponents(string: expectedRedirectURI),
            ...
            throw PearchHAOAuthSignInFailure.callback("invalid OAuth redirect URI")
        }
        guard callbackURL.scheme?.caseInsensitiveCompare(expectedCallbackURLScheme) == .orderedSame else {
            throw PearchHAOAuthSignInFailure.callback("OAuth callback scheme did not match")
        }
        guard configuration?.acceptsCallbackURL(callbackURL) == true else {
            throw PearchHAOAuthSignInFailure.callback("OAuth callback redirect URI did not match")
        }
        guard let components = URLComponents(url: callbackURL, resolvingAgainstBaseURL: false) else {
            throw PearchHAOAuthSignInFailure.callback("invalid OAuth callback")
        }
        ...
        if let error = values["error"] {
            ...
            throw PearchHAOAuthSignInFailure.callback("\(error)\(description)")
        }
        guard values["state"] == expectedState else {
            throw PearchHAOAuthSignInFailure.callback("OAuth state did not match")
        }
        guard let code = values["code"], !code.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            throw PearchHAOAuthSignInFailure.callback("OAuth callback is missing code")
        }
        return PearchHAOAuthCallback(code: code)
    }
...
private struct PearchHAOAuthCallback: Sendable {
    let code: String
}

@MainActor
public final class PearchHAASWebAuthenticationSessionPresenter: NSObject, PearchHAOAuthAuthorizationPresenter, ASWebAuthenticationPresentationContextProviding, @unchecked Sendable {
...
public final class PearchHAApplication: NSObject, NSApplicationDelegate {
    ...
    private var panelModel: PearchHAPanelModel?
    private var autoConnectTask: Task<Void, Never>?
    private let configStore: ConfigStore?
    private let authSessionStore: (any PearchHAAuthSessionStorage)?
    private let connector: PearchHAPanelModel.Connector
    private let historyProvider: PearchHAPanelModel.HistoryProvider
    private let bulkHistoryProvider: PearchHAPanelModel.BulkHistoryProvider
    private let serviceMetadataProvider: PearchHAPanelModel.ServiceMetadataProvider
    private let liveUpdateStreamer: PearchHAPanelModel.LiveUpdateStreamer?
    private let actionRunner: PearchHAPanelModel.ActionRunner
    private let oauthSignInRunner: PearchHAPanelModel.OAuthSignInRunner
    private let releaseUpdateChecker: PearchHAPanelModel.ReleaseUpdateChecker
    private let protectedActionValueStore: any ProtectedActionValueStore
    private let oauthApplicationConfiguration: PearchHAOAuthApplicationConfiguration?
    private let menuBarPresenter: PearchHAMenuBarPresenter
    private let gaugeImageRenderer: any PearchHAStatusItemGaugeImageRendering
    private let logoImageRenderer = PearchHAStatusItemLogoImageRenderer()
    private let performanceSignposter = OSSignposter(
        logger: Logger(subsystem: "dev.yuna.pearchha", category: "Performance")
    )
    private var statusItemLogoImageCache: NSImage?
    private var configuration = PearchHAConfiguration.empty
    private var configurationPersistenceState = PearchHAConfigurationPersistenceState.unavailable
    private var lastExternalURLEvent: PearchHAExternalURLEvent?
    ...
    public convenience init(configStore: ConfigStore?) {
        let authSessionStore = PearchHAAuthSessionStore()
        let gateway = PearchHAAuthorizedHomeAssistantGateway(authSessionStore: authSessionStore)
        let oauthSignInRunner: PearchHAPanelModel.OAuthSignInRunner
        let oauthApplicationConfiguration: PearchHAOAuthApplicationConfiguration?
        do {
            let configuration = try PearchHAOAuthApplicationConfiguration.fromEnvironment()
            let signInCoordinator = PearchHAOAuthSignInCoordinator(
        ...
        self.init(
            ...
            releaseUpdateChecker: { currentVersion in
                await PearchHAGitHubReleaseUpdateChecker().checkLatest(currentVersion: currentVersion)
            },
    ...
    public convenience init(
        configStore: ConfigStore?,
        authSessionStore: any PearchHAAuthSessionStorage,
        client: any PearchHARefreshingHomeAssistantClient = HomeAssistantClient()
    ) {
        let gateway = PearchHAAuthorizedHomeAssistantGateway(
            client: client,
            authSessionStore: authSessionStore
        )
        self.init(
    ...
    public convenience init(
        configStore: ConfigStore?,
        connector: @escaping PearchHAPanelModel.Connector,
        serviceMetadataProvider: @escaping PearchHAPanelModel.ServiceMetadataProvider = { _ in .success([]) },
        protectedActionValueStore: any ProtectedActionValueStore = KeychainProtectedActionValueStore(),
        authSessionStore: (any PearchHAAuthSessionStorage)? = nil,
        oauthApplicationConfiguration: PearchHAOAuthApplicationConfiguration? = nil,
        menuBarPresenter: PearchHAMenuBarPresenter = PearchHAMenuBarPresenter(),
        gaugeImageRenderer: any PearchHAStatusItemGaugeImageRendering = PearchHAStatusItemGaugeImageRenderer()
    ) {
        self.init(
            configStore: configStore,
            authSessionStore: authSessionStore,
            connector: connector,
            historyProvider: PearchHAApplication.history(form:entityID:range:),
            serviceMetadataProvider: serviceMetadataProvider,
            actionRunner: PearchHAApplication.action(form:action:),
    ...
    public init(
        ...
        authSessionStore: (any PearchHAAuthSessionStorage)? = nil,
        connector: @escaping PearchHAPanelModel.Connector,
        historyProvider: @escaping PearchHAPanelModel.HistoryProvider,
        bulkHistoryProvider: @escaping PearchHAPanelModel.BulkHistoryProvider = { _, _, _ in [:] },
        serviceMetadataProvider: @escaping PearchHAPanelModel.ServiceMetadataProvider = { _ in .success([]) },
        liveUpdateStreamer: PearchHAPanelModel.LiveUpdateStreamer? = nil,
        actionRunner: @escaping PearchHAPanelModel.ActionRunner = PearchHAApplication.action(form:action:),
        releaseUpdateChecker: @escaping PearchHAPanelModel.ReleaseUpdateChecker = { currentVersion in
            await PearchHAGitHubReleaseUpdateChecker().checkLatest(currentVersion: currentVersion)
        },
        protectedActionValueStore: any ProtectedActionValueStore = KeychainProtectedActionValueStore(),
        oauthSignInRunner: @escaping PearchHAPanelModel.OAuthSignInRunner = { _ in .failed("OAuth sign-in is not configured") },
        oauthApplicationConfiguration: PearchHAOAuthApplicationConfiguration? = nil,
        menuBarPresenter: PearchHAMenuBarPresenter = PearchHAMenuBarPresenter(),
        gaugeImageRenderer: any PearchHAStatusItemGaugeImageRendering = PearchHAStatusItemGaugeImageRenderer()
    ) {
    ...
    public static func main() {
        ...
        let delegate = PearchHAApplication(configStore: try? JSONConfigStore())
    ...
    @discardableResult
    private func startAutoConnect(form: PearchHAConnectionForm, model: PearchHAPanelModel) -> Task<Void, Never>? {
    ...
    private func storedSessionFormIfAvailable(for form: PearchHAConnectionForm) -> PearchHAConnectionForm? {
    ...
    private func shouldStopAutoConnectRetry(
        ...
        originalForm: PearchHAConnectionForm
    ) -> Bool {
    ...
    private func scheduleDisplayConfigurationRepairIfNeeded(after snapshot: PearchHAPanelSnapshot) {
    ...
    private func displayConfigurationRepairIsNeeded(after snapshot: PearchHAPanelSnapshot) -> Bool {
    ...
    @discardableResult
    public func handleExternalURLs(_ urls: [URL]) -> [PearchHAExternalURLEvent] {
    ...
    public var snapshot: PearchHAApplicationSnapshot {
        ...
        return PearchHAApplicationSnapshot(
            ...
            connectionForm: panelModel?.snapshot.connectionForm ?? PearchHAConnectionForm(),
    ...
    public func updateConnectionForm(
        ...
        addresses: [PearchHAConnectionAddressField]? = nil,
        token: String? = nil,
    ...
    @discardableResult
    public func setCoverControl(_ id: EntityID, command: PearchHACoverCommand) async -> Bool {
    ...
    public func setCustomActionServiceDataValue(
        _ id: CustomActionID,
        path: [PearchHACustomActionServiceDataPathComponent],
        value: ActionValue
    ) -> Bool {
    ...
    public func setCustomActionServiceDataText(
        ...
        key: String,
        text: String,
        kind: PearchHACustomActionServiceDataValueKind
    ) -> Bool {
    ...
    @discardableResult
    public func setMenuBarAppearance(_ id: EntityID, appearance: PearchHAMenuBarAppearance?) -> Bool {
    ...
    private func applyAppearancePreferences() {
        ...
        PearchHATheme.apply(accentColor: configuration.accentColor)
    }

    private func appearance(for themeMode: PearchHAThemeMode) -> NSAppearance? {
        switch themeMode {
        case .system:
            nil
        case .light:
    ...
    @discardableResult
    public func persist(displayPreferences preferences: PearchHADisplayPreferences) -> SelectionPersistenceResult {
        guard let configStore else {
            configurationPersistenceState = .unavailable
            return .failed("configuration store unavailable")
        }
    ...
    public static func makeSettingsWindow(
        model: PearchHAPanelModel,
        initialTab: PearchHASettingsView.Tab = .connection,
        initiallyExpandedEntityIDs: Set<EntityID> = [],
        displayPreferencesProvider: @escaping () -> PearchHADisplayPreferences = { .defaults },
        displayPreferencesSink: @escaping (PearchHADisplayPreferences) -> Void = { _ in },
        launchAtLoginProvider: @escaping () -> Bool = { false },
        launchAtLoginSink: @escaping (Bool) -> Bool = { _ in false }
    ) -> NSWindow {
        let window = PearchHASettingsWindow(
        ...
        window.contentViewController = PearchHAFirstMouseHostingController(
            rootView: PearchHASettingsView(
                model: model,
    ...
    private func openSettingsWindow(
        ...
        if let window = window as? PearchHASettingsWindow {
    ...
    private func updateStatusItems(from snapshot: PearchHAPanelSnapshot) {
        ...
        while statusItems.count < presentations.count {
            statusItems.append(PearchHAStatusItemEntry(item: makeStatusItem()))
        }
    ...
    private func statusItemAlreadyRendered(_ entry: PearchHAStatusItemEntry) -> Bool {
    ...
    private func apply(_ presentation: PearchHAMenuBarPresentation, to entry: inout PearchHAStatusItemEntry) {
        var image: NSImage?
        var title: String
    ...
    private func shouldShowStatusItemLoadingIndicator(for snapshot: PearchHAPanelSnapshot) -> Bool {
    ...
    private func updateLoadingIndicator(
        shouldShow: Bool,
        for entry: inout PearchHAStatusItemEntry
    ) {
        guard let button = entry.item.button else {
            return
        }
    ...
    private func cachedStatusItemImage(for item: RenderedMenuBarItem, in entry: inout PearchHAStatusItemEntry) -> NSImage? {
        let key = PearchHAStatusItemImageCacheKey(item: item)
    ...
    private func statusItemSnapshot(for item: NSStatusItem) -> PearchHAMenuBarStatusItemSnapshot {
        ...
        return PearchHAMenuBarStatusItemSnapshot(
            title: title,
    ...
    private func loadConfiguration() -> PearchHAConfiguration {
        guard let configStore else {
            configurationPersistenceState = .unavailable
            return .empty
        }
    ...
    private func restoredConnectionForm() -> PearchHAConnectionForm {
        ...
        let alternativeFields = profileAddresses.dropFirst().map { address in
            PearchHAConnectionAddressField(label: address.label, urlString: address.urlString)
        }
        return PearchHAConnectionForm(
    ...
    private func rememberConnection(_ form: PearchHAConnectionForm) {
        if !form.trimmedToken.isEmpty {
        ...
        let nextConfiguration = configuration.replacing(
            connectionProfile: PearchHAConnectionProfile(
                addresses: [
                    PearchHAConnectionAddress(
                        urlString: PearchHAConnectionForm.normalizedHomeAssistantURLString(form.urlString)
                    )
                ] + form.addresses.map { address in
                    PearchHAConnectionAddress(
                        label: address.label,
                        urlString: PearchHAConnectionForm.normalizedHomeAssistantURLString(address.urlString)
                    )
                },
                allowsSelfSignedCertificates: form.allowsSelfSignedCertificates
            )
    ...
    private func rememberAttemptedManualToken(_ form: PearchHAConnectionForm) {
        rememberAccessToken(form.trimmedToken)
    }

    private func shouldRememberAttemptedManualToken(
        form: PearchHAConnectionForm,
        result: PearchHAConnectionAttemptResult
    ) -> Bool {
    ...
    private func externalURLEvent(for url: URL) -> PearchHAExternalURLEvent {
        let components = URLComponents(url: url, resolvingAgainstBaseURL: false)
        let scheme = components?.scheme ?? url.scheme
        let disposition: PearchHAExternalURLDisposition
        ...
        return PearchHAExternalURLEvent(
            scheme: scheme,
    ...
    private static func connect(form: PearchHAConnectionForm) async -> PearchHAConnectionAttemptResult {
        await PearchHAAuthorizedHomeAssistantGateway().connect(form: form)
    }
    ...
    private static func history(
        ...
        await PearchHAAuthorizedHomeAssistantGateway().history(form: form, entityID: entityID, range: range)
    }

    public static func services(form: PearchHAConnectionForm) async -> PearchHAServiceMetadataProviderResult {
        await PearchHAAuthorizedHomeAssistantGateway().services(form: form)
    }

    public static func action(form: PearchHAConnectionForm, action: ActionSpec) async -> PearchHAActionResult {
        await PearchHAAuthorizedHomeAssistantGateway().action(form: form, action: action)
    }
}
...
private struct PearchHAStatusItemEntry {
    let item: NSStatusItem
    var presentation: PearchHAMenuBarPresentation = .fallback
    var imageCache: (key: PearchHAStatusItemImageCacheKey, image: NSImage?)?
    weak var loadingIndicator: NSProgressIndicator?
}

private struct PearchHAStatusItemImageCacheKey: Equatable {
