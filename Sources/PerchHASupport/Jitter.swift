...
public struct JitteredScheduler: Sendable {
    public let base: PerchDuration
    public let fraction: Double
    private var generator: SeededGenerator

    public init(base: PerchDuration, fraction: Double, seed: UInt64) {
    ...
    public mutating func nextDelay() -> PerchDuration {
        let span = Double(base.nanoseconds) * fraction
        let offset = Int64((generator.nextUnit() * 2 - 1) * span)
        return PerchDuration(nanoseconds: base.nanoseconds + offset)
    }
}
...
public struct ExponentialBackoff: Sendable {
    public let base: PerchDuration
    public let cap: PerchDuration
    public let multiplier: Double
    private var generator: SeededGenerator

    public init(base: PerchDuration, cap: PerchDuration, multiplier: Double = 2, seed: UInt64) {
    ...
    public mutating func delay(forAttempt attempt: Int) -> PerchDuration {
        ...
        return PerchDuration(nanoseconds: Int64(generator.nextUnit() * raw))
    }
}
