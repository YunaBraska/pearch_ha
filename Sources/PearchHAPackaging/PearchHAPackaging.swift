...
public enum PearchHAAppBundleManifestError: Error, Equatable, CustomStringConvertible, Sendable {
...
public struct PearchHAAppBundleManifest: Equatable, Sendable {
    ...
    public init(
        ...
        bundleIdentifier: String = "dev.pearchha.app",
        executableName: String = "PearchHA",
        version: String = "0.1.0",
        buildVersion: String = "1",
        minimumSystemVersion: String = "13.0",
        callbackURLScheme: String = "pearchha",
        ...
        guard Self.isValidURLScheme(normalizedScheme) else {
            throw PearchHAAppBundleManifestError.invalidCallbackURLScheme(normalizedScheme)
        }
    ...
    private static func required(_ value: String, name: String) throws -> String {
        ...
        guard !trimmed.isEmpty else {
            throw PearchHAAppBundleManifestError.blankField(name)
        }
        return trimmed
    }
...
public enum PearchHAAppBundleBuildError: Error, Equatable, CustomStringConvertible, Sendable {
...
public struct PearchHAAppBundleBuildConfiguration: Equatable, Sendable {
    public let executableURL: URL
    public let outputURL: URL
    public let manifest: PearchHAAppBundleManifest
    ...
    public init(
        executableURL: URL,
        outputURL: URL,
        manifest: PearchHAAppBundleManifest,
        iconURL: URL? = nil,
...
public struct PearchHAAppBundleBuildResult: Equatable, Sendable {
    public let appURL: URL
    public let executableURL: URL
    public let infoPlistURL: URL
}

public enum PearchHAOAuthClientWebsiteError: Error, Equatable, CustomStringConvertible, Sendable {
    case blankField(String)
...
public struct PearchHAOAuthClientWebsiteManifest: Equatable, Sendable {
    ...
    public init(
        ...
        guard Self.isValidClientWebsiteURL(normalizedClientID) else {
            throw PearchHAOAuthClientWebsiteError.invalidClientID(normalizedClientID)
        }
        guard Self.isValidRedirectURI(normalizedRedirectURI) else {
            throw PearchHAOAuthClientWebsiteError.invalidRedirectURI(normalizedRedirectURI)
        }
    ...
    private static func required(_ value: String, name: String) throws -> String {
        let trimmed = value.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else {
            throw PearchHAOAuthClientWebsiteError.blankField(name)
        }
        return trimmed
    }
...
public struct PearchHAOAuthClientWebsiteBuildConfiguration: Equatable, Sendable {
    public let outputURL: URL
    public let manifest: PearchHAOAuthClientWebsiteManifest
    public let replaceExisting: Bool

    public init(
        outputURL: URL,
        manifest: PearchHAOAuthClientWebsiteManifest,
        replaceExisting: Bool = false
    ) {
        self.outputURL = outputURL
        self.manifest = manifest
...
public struct PearchHAOAuthClientWebsiteBuildResult: Equatable, Sendable {
    public let siteURL: URL
    public let clientID: String
    public let redirectURI: String
}

public struct PearchHAOAuthClientWebsiteBuilder {
    ...
    public func build(
        _ configuration: PearchHAOAuthClientWebsiteBuildConfiguration
    ) throws -> PearchHAOAuthClientWebsiteBuildResult {
        let outputURL = configuration.outputURL
        if fileManager.fileExists(atPath: outputURL.path) {
            guard configuration.replaceExisting else {
                throw PearchHAOAuthClientWebsiteError.outputExists(outputURL.path)
            }
        ...
        return PearchHAOAuthClientWebsiteBuildResult(
            siteURL: outputURL,
...
public struct PearchHAOAuthClientWebsiteVerificationConfiguration: Equatable, Sendable {
    public let siteURL: URL
    public let manifest: PearchHAOAuthClientWebsiteManifest

    public init(siteURL: URL, manifest: PearchHAOAuthClientWebsiteManifest) {
        self.siteURL = siteURL
        self.manifest = manifest
    }
}

public struct PearchHAOAuthClientWebsiteVerificationResult: Equatable, Sendable {
    public let siteURL: URL
    public let clientID: String
    public let redirectURI: String
}

public struct PearchHAOAuthClientWebsiteVerifier {
    private let fileManager: FileManager
    ...
    public func verify(
        _ configuration: PearchHAOAuthClientWebsiteVerificationConfiguration
    ) throws -> PearchHAOAuthClientWebsiteVerificationResult {
        let siteURL = configuration.siteURL
        guard fileManager.fileExists(atPath: siteURL.path) else {
            throw PearchHAOAuthClientWebsiteError.siteMissing(siteURL.path)
        }
        let data: Data
        do {
            data = try Data(contentsOf: siteURL)
        } catch {
            throw PearchHAOAuthClientWebsiteError.siteUnreadable(siteURL.path)
        }
        return try verify(
            data: data,
            siteURL: siteURL,
    ...
    public func verify(
        ...
        guard html.contains(manifest.canonicalLinkHTML) else {
            throw PearchHAOAuthClientWebsiteError.canonicalLinkMissing(manifest.clientID)
        }
        let prefix = String(decoding: data.prefix(10_000), as: UTF8.self)
        guard prefix.contains(manifest.redirectLinkHTML) else {
            throw PearchHAOAuthClientWebsiteError.redirectDeclarationMissing(manifest.redirectURI)
        }
        return PearchHAOAuthClientWebsiteVerificationResult(
            siteURL: siteURL,
...
public struct PearchHAAppBundleBuilder {
    private let fileManager: FileManager
    ...
    public func build(_ configuration: PearchHAAppBundleBuildConfiguration) throws -> PearchHAAppBundleBuildResult {
        guard fileManager.fileExists(atPath: configuration.executableURL.path) else {
            throw PearchHAAppBundleBuildError.executableMissing(configuration.executableURL.path)
        }
        if let iconURL = configuration.iconURL,
           !fileManager.fileExists(atPath: iconURL.path)
        {
            throw PearchHAAppBundleBuildError.iconMissing(iconURL.path)
        }
        if fileManager.fileExists(atPath: configuration.outputURL.path) {
            guard configuration.replaceExisting else {
                throw PearchHAAppBundleBuildError.outputExists(configuration.outputURL.path)
            }
        ...
        return PearchHAAppBundleBuildResult(
...
public enum PearchHALaunchServicesVerificationError: Error, Equatable, CustomStringConvertible, Sendable {
...
public struct PearchHALaunchServicesVerificationConfiguration: Equatable, Sendable {
    public let appURL: URL
    public let callbackURLScheme: String
...
public struct PearchHALaunchServicesVerificationResult: Equatable, Sendable {
    public let appURL: URL
...
public struct PearchHALaunchServicesVerifier {
    ...
    public init(
        ...
        toolURL: URL = PearchHALaunchServicesVerifier.defaultToolURL
    ) {
        self.fileManager = fileManager
    ...
    public func verify(
        _ configuration: PearchHALaunchServicesVerificationConfiguration
    ) throws -> PearchHALaunchServicesVerificationResult {
        ...
        guard fileManager.fileExists(atPath: appURL.path, isDirectory: &isDirectory), isDirectory.boolValue else {
            throw PearchHALaunchServicesVerificationError.appBundleMissing(appURL.path)
        }
        ...
        guard fileManager.fileExists(atPath: infoPlistURL.path) else {
            throw PearchHALaunchServicesVerificationError.infoPlistMissing(infoPlistURL.path)
        }
        ...
        guard let urlType else {
            throw PearchHALaunchServicesVerificationError.callbackSchemeMissing(configuration.callbackURLScheme)
        }
        guard urlType.role == Self.defaultURLTypeRole else {
            throw PearchHALaunchServicesVerificationError.callbackRoleMissing(Self.defaultURLTypeRole)
        }
        ...
        return PearchHALaunchServicesVerificationResult(
            appURL: appURL,
    ...
    private func infoPlistCallbackURLType(
        infoPlistURL: URL,
        callbackURLScheme: String
    ) throws -> PearchHAInfoPlistURLTypeSnapshot? {
        let value: Any
        do {
            ...
            throw PearchHALaunchServicesVerificationError.infoPlistUnreadable(infoPlistURL.path)
        }
        ...
        for urlType in urlTypes {
            ...
            if schemes.contains(where: { $0.caseInsensitiveCompare(callbackURLScheme) == .orderedSame }) {
                return PearchHAInfoPlistURLTypeSnapshot(
                    schemes: schemes,
    ...
    private func callbackURL(scheme: String) throws -> URL {
        guard let url = URL(string: "\(scheme)://auth") else {
            throw PearchHALaunchServicesVerificationError.invalidCallbackURL(scheme)
        }
        return url
    }
    ...
    private func launchServicesDump(appURL: URL, shouldRegister: Bool) throws -> String {
        guard fileManager.isExecutableFile(atPath: toolURL.path) else {
            throw PearchHALaunchServicesVerificationError.launchServicesToolMissing(toolURL.path)
        }
        if shouldRegister {
    ...
    private func launchServicesOutput(arguments: [String]) throws -> String {
        ...
        guard process.terminationStatus == 0 else {
            throw PearchHALaunchServicesVerificationError.launchServicesDumpFailed(
    ...
    private func verifyLaunchServicesDump(
        ...
        guard let appPathRange = dump.range(of: appURL.path) else {
            throw PearchHALaunchServicesVerificationError.launchServicesBundleMissing(appURL.path)
        }
        ...
        guard appClaimSlice.range(
            ...
            throw PearchHALaunchServicesVerificationError.launchServicesSchemeMissing(callbackURLScheme)
        }
        ...
        guard claimBlock.range(
            ...
            throw PearchHALaunchServicesVerificationError.launchServicesRoleMissing(Self.defaultURLTypeRole)
        }
    }
...
public struct PearchHACommandResult: Equatable, Sendable {
...
public enum PearchHAReleasePreflightIssue: Error, Equatable, CustomStringConvertible, Sendable {
...
public struct PearchHAReleasePreflightConfiguration: Equatable, Sendable {
...
public struct PearchHAReleasePreflightReport: Equatable, Sendable {
    public let signingIdentityAvailable: Bool
    public let notaryProfileAvailable: Bool
    public let issues: [PearchHAReleasePreflightIssue]

    public init(
        signingIdentityAvailable: Bool,
        notaryProfileAvailable: Bool,
        issues: [PearchHAReleasePreflightIssue]
    ) {
    ...
    public func diagnostic(
        configuration: PearchHAReleasePreflightConfiguration
    ) -> PearchHAReleasePreflightDiagnostic {
        PearchHAReleasePreflightDiagnostic(report: self, configuration: configuration)
    }

    public func nextSteps(configuration: PearchHAReleasePreflightConfiguration) -> [String] {
    ...
    public func suggestedCommands(configuration: PearchHAReleasePreflightConfiguration) -> [String] {
        var commands: [String] = []
        if issues.contains(where: Self.isSigningIdentityIssue) {
        ...
        if issues.contains(where: Self.isNotaryProfileIssue) {
            commands.append(
                "xcrun notarytool history --keychain-profile \(PearchHAReleasePreflightGuidance.exampleNotaryProfile) --output-format json --no-progress"
            )
        }
        if isReadyForCredentialedRelease {
            commands.append(PearchHAReleasePreflightGuidance.credentialedPackageCommand)
        } else {
            commands.append(PearchHAReleasePreflightGuidance.preflightCommand(json: true))
        }
        return commands
    }

    private static func isDeveloperToolIssue(_ issue: PearchHAReleasePreflightIssue) -> Bool {
        switch issue {
    ...
    private static func isSigningIdentityIssue(_ issue: PearchHAReleasePreflightIssue) -> Bool {
        switch issue {
    ...
    private static func isNotaryProfileIssue(_ issue: PearchHAReleasePreflightIssue) -> Bool {
        switch issue {
...
public enum PearchHAReleasePreflightPresence: String, Codable, Equatable, Sendable {
    case present
    case missing
...
public enum PearchHAReleasePreflightState: String, Codable, Equatable, Sendable {
    case ready
    case blocked
...
public struct PearchHAReleasePreflightDiagnosticIssue: Codable, Equatable, Sendable {
...
public struct PearchHAReleasePreflightDiagnostic: Codable, Equatable, Sendable {
    public let signingIdentity: PearchHAReleasePreflightPresence
    public let notaryProfile: PearchHAReleasePreflightPresence
    public let developerIDIdentity: PearchHAReleasePreflightState
    public let notaryKeychainProfile: PearchHAReleasePreflightState
    public let credentialedRelease: PearchHAReleasePreflightState
    public let issues: [PearchHAReleasePreflightDiagnosticIssue]
    public let nextSteps: [String]
    public let suggestedCommands: [String]

    public init(
        signingIdentity: PearchHAReleasePreflightPresence,
        notaryProfile: PearchHAReleasePreflightPresence,
        developerIDIdentity: PearchHAReleasePreflightState,
        notaryKeychainProfile: PearchHAReleasePreflightState,
        credentialedRelease: PearchHAReleasePreflightState,
        issues: [PearchHAReleasePreflightDiagnosticIssue],
        nextSteps: [String],
    ...
    public init(
        report: PearchHAReleasePreflightReport,
        configuration: PearchHAReleasePreflightConfiguration
    ) {
        signingIdentity = PearchHAReleasePreflightPresence(
        ...
        developerIDIdentity = PearchHAReleasePreflightState(report.signingIdentityAvailable)
        notaryKeychainProfile = PearchHAReleasePreflightState(report.notaryProfileAvailable)
        credentialedRelease = PearchHAReleasePreflightState(report.isReadyForCredentialedRelease)
        issues = report.issues.map {
            PearchHAReleasePreflightDiagnosticIssue(
...
public struct PearchHAReleasePreflightChecker {
    ...
    public init(
        ...
        xcrunURL: URL = PearchHANotarySubmitter.defaultXcrunURL,
        commandRunner: any PearchHACommandRunning = PearchHAFoundationCommandRunner()
    ) {
        self.fileManager = fileManager
    ...
    public func check(_ configuration: PearchHAReleasePreflightConfiguration) throws -> PearchHAReleasePreflightReport {
        var issues: [PearchHAReleasePreflightIssue] = []
        ...
        return PearchHAReleasePreflightReport(
    ...
    private func checkSigningIdentity(
        ...
        issues: inout [PearchHAReleasePreflightIssue]
    ) throws -> Bool {
...
public enum PearchHAReleasePreflightGuidance {
    public static let exampleSigningIdentity = #"Developer ID Application: Example (TEAMID)"#
    public static let exampleNotaryProfile = "pearchha-release"

    public static func preflightCommand(json: Bool) -> String {
        let jsonFlag = json ? " --json" : ""
        return #"swift run pearchha-package-app --release-preflight\#(jsonFlag) --sign-identity "\#(exampleSigningIdentity)" --notary-profile \#(exampleNotaryProfile)"#
    }

    public static var credentialedPackageCommand: String {
        #"""
        swift run pearchha-package-app --executable .build/release/PearchHA --output .build/PearchHA.app --callback-scheme pearchha --replace --verify-launch-services --sign-identity "\#(exampleSigningIdentity)" --verify-signature --package-dmg --verify-dmg --verify-dmg-contents --notary-profile \#(exampleNotaryProfile) --release-manifest .build/pearchha-release-manifest.json --snapshot-dir .build/pearchha-snapshots/current
        """#
    }
}

public protocol PearchHACommandRunning: Sendable {
    func run(executableURL: URL, arguments: [String]) throws -> PearchHACommandResult
}

public struct PearchHAFoundationCommandRunner: PearchHACommandRunning {
    public init() {}

    public func run(executableURL: URL, arguments: [String]) throws -> PearchHACommandResult {
        let process = Process()
...
public enum PearchHAReleaseEvidenceError: Error, Equatable, CustomStringConvertible, Sendable {
    case appBundleMissing(String)
    case infoPlistMissing(String)
    case infoPlistUnreadable(String)
...
public enum PearchHAReleaseEvidenceScreenshots {
...
public enum PearchHAReleaseEvidenceReview {
...
public struct PearchHAReleaseEvidenceConfiguration: Equatable, Sendable {
    public let appURL: URL
...
public struct PearchHAReleaseEvidenceManifest: Codable, Equatable, Sendable {
    public let schemaVersion: Int
    public let app: PearchHAReleaseEvidenceApp
    public let dmg: PearchHAReleaseEvidenceArtifact
    public let oauthClientWebsite: PearchHAReleaseEvidenceArtifact?
    public let screenshots: [PearchHAReleaseEvidenceArtifact]
    public let reviewBaseline: PearchHAReleaseEvidenceArtifact?
    public let signing: PearchHAReleaseEvidenceSigning
    public let notarization: PearchHAReleaseEvidenceNotarization
}

public struct PearchHAReleaseEvidenceApp: Codable, Equatable, Sendable {
...
public struct PearchHAReleaseEvidenceArtifact: Codable, Equatable, Sendable {
    public let name: String
    public let path: String
...
public struct PearchHAReleaseEvidenceSigning: Codable, Equatable, Sendable {
    public let identity: String?
    public let adHoc: Bool
    public let signatureVerified: Bool
}

public struct PearchHAReleaseEvidenceNotarization: Codable, Equatable, Sendable {
...
private func releaseStatusProblem(
    signing: PearchHAReleaseEvidenceSigning,
    notarization: PearchHAReleaseEvidenceNotarization
) -> String? {
...
private func firstMissingRequiredScreenshotName(
    ...
    screenshots: [PearchHAReleaseEvidenceArtifact]
) -> String? {
...
public enum PearchHAReleaseEvidenceVerificationError: Error, Equatable, CustomStringConvertible, Sendable {
...
public struct PearchHAReleaseEvidenceVerificationConfiguration: Equatable, Sendable {
    public let manifestURL: URL
    public let requiredScreenshotNames: [String]
...
public struct PearchHAReleaseEvidenceVerificationReport: Equatable, Sendable {
    public let manifestURL: URL
    public let appURL: URL
    public let dmgURL: URL
...
public enum PearchHAReleaseEvidenceBundleError: Error, Equatable, CustomStringConvertible, Sendable {
    case manifestMissing(String)
    case manifestUnreadable(String)
    case outputExists(String)
...
public struct PearchHAReleaseEvidenceBundleConfiguration: Equatable, Sendable {
    public let manifestURL: URL
...
public struct PearchHAReleaseEvidenceBundleResult: Equatable, Sendable {
    public let bundleDirectoryURL: URL
    public let manifestURL: URL
}

public struct PearchHAReleaseEvidenceVerifier {
    private let fileManager: FileManager
    ...
    public init(
        fileManager: FileManager = .default,
        codesignURL: URL = PearchHACodeSigner.defaultCodesignURL,
        xcrunURL: URL = PearchHANotarySubmitter.defaultXcrunURL,
        commandRunner: any PearchHACommandRunning = PearchHAFoundationCommandRunner()
    ) {
        self.fileManager = fileManager
    ...
    @discardableResult
    public func verify(
        _ configuration: PearchHAReleaseEvidenceVerificationConfiguration
    ) throws -> PearchHAReleaseEvidenceVerificationReport {
        let manifestURL = normalizedReleaseEvidenceURL(configuration.manifestURL, isDirectory: false)
        guard fileManager.fileExists(atPath: manifestURL.path) else {
            throw PearchHAReleaseEvidenceVerificationError.manifestMissing(manifestURL.path)
        }
        let manifest: PearchHAReleaseEvidenceManifest
        do {
            ...
            throw PearchHAReleaseEvidenceVerificationError.manifestUnreadable(manifestURL.path)
        }

        guard manifest.schemaVersion == 1 else {
            throw PearchHAReleaseEvidenceVerificationError.unsupportedSchemaVersion(manifest.schemaVersion)
        }
        try verifyStatus(manifest)
        guard !manifest.screenshots.isEmpty else {
            throw PearchHAReleaseEvidenceVerificationError.screenshotsMissing(manifestURL.path)
        }
        ...
        return PearchHAReleaseEvidenceVerificationReport(
            manifestURL: manifestURL,
    ...
    private func verifyStatus(_ manifest: PearchHAReleaseEvidenceManifest) throws {
        if let problem = releaseStatusProblem(signing: manifest.signing, notarization: manifest.notarization) {
            throw PearchHAReleaseEvidenceVerificationError.invalidReleaseStatus(problem)
        }
    }
    ...
    private func verifyRequiredScreenshots(
        ...
        if let name = firstMissingRequiredScreenshotName(requiredNames, screenshots: screenshots) {
            throw PearchHAReleaseEvidenceVerificationError.requiredScreenshotMissing(
                name: name,
    ...
    private func verifyCodeSignatureIfRecorded(
        _ manifest: PearchHAReleaseEvidenceManifest,
        appURL: URL
    ) throws {
        ...
        do {
            _ = try PearchHACodeSignatureVerifier(
                fileManager: fileManager,
                codesignURL: codesignURL,
                commandRunner: commandRunner
            ).verify(
                PearchHACodeSignatureVerificationConfiguration(
                    appURL: appURL
                )
            )
        } catch {
            throw PearchHAReleaseEvidenceVerificationError.codeSignatureVerificationFailed(String(describing: error))
        }
    }
    ...
    private func verifyStapledArtifactIfRecorded(
        ...
        guard fileManager.isExecutableFile(atPath: xcrunURL.path) else {
            throw PearchHAReleaseEvidenceVerificationError.xcrunMissing(xcrunURL.path)
        }
        let result = try commandRunner.run(
            executableURL: xcrunURL,
        ...
        guard result.status == 0 else {
            throw PearchHAReleaseEvidenceVerificationError.staplerValidationFailed(
    ...
    private func verifyApp(
        expected: PearchHAReleaseEvidenceApp,
        expectedURL: URL,
        actual: PearchHAReleaseEvidenceApp
    ) throws {
    ...
    private func verifyAppField(
        ...
        guard expected == actual else {
            throw PearchHAReleaseEvidenceVerificationError.appMetadataMismatch(
                field: field,
                expected: expected,
                actual: actual
            )
    ...
    private func verifyArtifact(
        _ expected: PearchHAReleaseEvidenceArtifact,
        manifestURL: URL
    ) throws {
        let url = resolveReleaseEvidencePath(expected.path, manifestURL: manifestURL, isDirectory: false)
        guard fileManager.fileExists(atPath: url.path) else {
            throw PearchHAReleaseEvidenceVerificationError.artifactMissing(url.path)
        }
        let data = try Data(contentsOf: url)
        let actualSHA256 = sha256Hex(data)
        guard expected.sha256 == actualSHA256 else {
            throw PearchHAReleaseEvidenceVerificationError.artifactSHA256Mismatch(
                path: url.path,
        ...
        guard expected.byteCount == actualByteCount else {
            throw PearchHAReleaseEvidenceVerificationError.artifactByteCountMismatch(
                path: url.path,
    ...
    private func appEvidence(_ appURL: URL) throws -> PearchHAReleaseEvidenceApp {
        ...
        guard fileManager.fileExists(atPath: normalizedAppURL.path, isDirectory: &isDirectory), isDirectory.boolValue else {
            throw PearchHAReleaseEvidenceVerificationError.appBundleMissing(normalizedAppURL.path)
        }
        let infoPlistURL = normalizedAppURL.appendingPathComponent("Contents/Info.plist", isDirectory: false)
        guard fileManager.fileExists(atPath: infoPlistURL.path) else {
            throw PearchHAReleaseEvidenceVerificationError.infoPlistMissing(infoPlistURL.path)
        }
        ...
        do {
            ...
            throw PearchHAReleaseEvidenceVerificationError.infoPlistUnreadable(infoPlistURL.path)
        }
        guard let plist = value as? [String: Any] else {
            throw PearchHAReleaseEvidenceVerificationError.infoPlistUnreadable(infoPlistURL.path)
        }

        return PearchHAReleaseEvidenceApp(
...
public struct PearchHAReleaseEvidenceBundler {
    private let fileManager: FileManager
    ...
    public func bundle(
        _ configuration: PearchHAReleaseEvidenceBundleConfiguration
    ) throws -> PearchHAReleaseEvidenceBundleResult {
        let manifestURL = normalizedReleaseEvidenceURL(configuration.manifestURL, isDirectory: false)
        guard fileManager.fileExists(atPath: manifestURL.path) else {
            throw PearchHAReleaseEvidenceBundleError.manifestMissing(manifestURL.path)
        }
        let manifest: PearchHAReleaseEvidenceManifest
        do {
            manifest = try JSONDecoder().decode(
            ...
            throw PearchHAReleaseEvidenceBundleError.manifestUnreadable(manifestURL.path)
        }
        ...
        if fileManager.fileExists(atPath: outputDirectoryURL.path) {
            guard configuration.replaceExisting else {
                throw PearchHAReleaseEvidenceBundleError.outputExists(outputDirectoryURL.path)
            }
        ...
        return PearchHAReleaseEvidenceBundleResult(
    ...
    private func bundledManifest(
        from manifest: PearchHAReleaseEvidenceManifest,
        manifestURL: URL,
        outputDirectoryURL: URL,
        copiedDestinations: inout Set<String>
    ) throws -> PearchHAReleaseEvidenceManifest {
        ...
        let bundledApp = PearchHAReleaseEvidenceApp(
            path: bundledAppPath,
        ...
        return PearchHAReleaseEvidenceManifest(
    ...
    private func bundledArtifact(
        _ artifact: PearchHAReleaseEvidenceArtifact,
        manifestURL: URL,
        outputDirectoryURL: URL,
        copiedDestinations: inout Set<String>
    ) throws -> PearchHAReleaseEvidenceArtifact {
        ...
        return PearchHAReleaseEvidenceArtifact(
            name: artifact.name,
            path: bundledPath,
    ...
    private func copyAdjacentCurrentReviewBaselineIfPresent(
        ...
        guard sourceReviewBaselinePath.hasSuffix("/" + PearchHAReleaseEvidenceReview.expectedBaselineFilename)
              || sourceReviewBaselinePath == PearchHAReleaseEvidenceReview.expectedBaselineFilename
        else {
            return
        }
        ...
        let currentSourceURL = expectedSourceURL.deletingLastPathComponent()
            .appendingPathComponent(PearchHAReleaseEvidenceReview.currentBaselineFilename, isDirectory: false)
...
public struct PearchHAReleaseEvidenceWriter {
    private let fileManager: FileManager
    ...
    @discardableResult
    public func write(_ configuration: PearchHAReleaseEvidenceConfiguration) throws -> PearchHAReleaseEvidenceManifest {
        ...
        guard fileManager.fileExists(atPath: dmgURL.path) else {
            throw PearchHAReleaseEvidenceError.dmgMissing(dmgURL.path)
        }
        let signingIdentity = trimmedOptional(configuration.signingIdentity)
        let signing = PearchHAReleaseEvidenceSigning(
            identity: signingIdentity,
            adHoc: signingIdentity == "-",
            signatureVerified: configuration.signatureVerified
        )
        let notarization = PearchHAReleaseEvidenceNotarization(
        ...
        if let problem = releaseStatusProblem(signing: signing, notarization: notarization) {
            throw PearchHAReleaseEvidenceError.invalidReleaseStatus(problem)
        }

        let manifest = PearchHAReleaseEvidenceManifest(
            schemaVersion: 1,
            app: PearchHAReleaseEvidenceApp(
    ...
    private func appEvidence(_ appURL: URL) throws -> PearchHAReleaseEvidenceApp {
        var isDirectory: ObjCBool = false
        guard fileManager.fileExists(atPath: appURL.path, isDirectory: &isDirectory), isDirectory.boolValue else {
            throw PearchHAReleaseEvidenceError.appBundleMissing(appURL.path)
        }
        let infoPlistURL = appURL.appendingPathComponent("Contents/Info.plist", isDirectory: false)
        guard fileManager.fileExists(atPath: infoPlistURL.path) else {
            throw PearchHAReleaseEvidenceError.infoPlistMissing(infoPlistURL.path)
        }
        ...
        do {
            ...
            throw PearchHAReleaseEvidenceError.infoPlistUnreadable(infoPlistURL.path)
        }
        guard let plist = value as? [String: Any] else {
            throw PearchHAReleaseEvidenceError.infoPlistUnreadable(infoPlistURL.path)
        }
    ...
    private func screenshotEvidence(
        ...
        guard let directoryURL else {
            guard requiredNames.isEmpty else {
                throw PearchHAReleaseEvidenceError.screenshotDirectoryRequired
            }
            return []
        }
        ...
        guard fileManager.fileExists(atPath: directory.path, isDirectory: &isDirectory), isDirectory.boolValue else {
            throw PearchHAReleaseEvidenceError.screenshotDirectoryMissing(directory.path)
        }
        ...
        guard !urls.isEmpty else {
            throw PearchHAReleaseEvidenceError.screenshotsMissing(directory.path)
        }
        let screenshots = try urls.map { try artifactEvidence($0, relativeTo: manifestDirectoryURL) }
        if let name = firstMissingRequiredScreenshotName(requiredNames, screenshots: screenshots) {
            throw PearchHAReleaseEvidenceError.requiredScreenshotMissing(
                name: name,
    ...
    private func reviewBaselineEvidence(
        ...
        guard fileManager.fileExists(atPath: fileURL.path) else {
            throw PearchHAReleaseEvidenceError.reviewBaselineMissing(fileURL.path)
        }
...
public enum PearchHACodeSigningError: Error, Equatable, CustomStringConvertible, Sendable {
    case appBundleMissing(String)
    case blankIdentity
...
public struct PearchHACodeSigningConfiguration: Equatable, Sendable {
    public let appURL: URL
    public let identity: String
...
public struct PearchHACodeSigningResult: Equatable, Sendable {
    public let appURL: URL
    public let identity: String
    public let entitlementsURL: URL?
    public let hardenedRuntime: Bool
...
public struct PearchHACodeSigner {
    ...
    public func sign(_ configuration: PearchHACodeSigningConfiguration) throws -> PearchHACodeSigningResult {
        let appURL = normalizedFileURL(configuration.appURL)
        let identity = configuration.identity.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !identity.isEmpty else {
            throw PearchHACodeSigningError.blankIdentity
        }
        try verifyAppBundleExists(appURL)
        guard fileManager.isExecutableFile(atPath: codesignURL.path) else {
            throw PearchHACodeSigningError.codesignMissing(codesignURL.path)
        }
        ...
        if let entitlementsURL {
            guard fileManager.fileExists(atPath: entitlementsURL.path) else {
                throw PearchHACodeSigningError.entitlementsMissing(entitlementsURL.path)
            }
        }
        ...
        guard result.status == 0 else {
            throw PearchHACodeSigningError.codesignFailed(operation: "sign", status: result.status, output: result.output)
        }
        return PearchHACodeSigningResult(
            appURL: appURL,
            identity: identity,
    ...
    private func verifyAppBundleExists(_ appURL: URL) throws {
        var isDirectory: ObjCBool = false
        guard fileManager.fileExists(atPath: appURL.path, isDirectory: &isDirectory), isDirectory.boolValue else {
            throw PearchHACodeSigningError.appBundleMissing(appURL.path)
        }
    }
...
public struct PearchHACodeSignatureVerificationConfiguration: Equatable, Sendable {
    public let appURL: URL
...
public struct PearchHACodeSignatureVerificationResult: Equatable, Sendable {
    public let appURL: URL
}

public struct PearchHACodeSignatureVerifier {
    private let fileManager: FileManager
    private let codesignURL: URL
    private let commandRunner: any PearchHACommandRunning
    ...
    public func verify(
        _ configuration: PearchHACodeSignatureVerificationConfiguration
    ) throws -> PearchHACodeSignatureVerificationResult {
        let appURL = normalizedFileURL(configuration.appURL)
        var isDirectory: ObjCBool = false
        guard fileManager.fileExists(atPath: appURL.path, isDirectory: &isDirectory), isDirectory.boolValue else {
            throw PearchHACodeSigningError.appBundleMissing(appURL.path)
        ...
        let result = try commandRunner.run(executableURL: codesignURL, arguments: arguments)
        guard result.status == 0 else {
            throw PearchHACodeSigningError.codesignFailed(operation: "verify", status: result.status, output: result.output)
        }
        return PearchHACodeSignatureVerificationResult(appURL: appURL)
    }
...
public enum PearchHANotaryError: Error, Equatable, CustomStringConvertible, Sendable {
    case artifactMissing(String)
...
public struct PearchHANotarySubmissionConfiguration: Equatable, Sendable {
...
public struct PearchHANotarySubmissionResult: Equatable, Sendable {
    public let artifactURL: URL
    public let keychainProfile: String
...
public struct PearchHANotarySubmitter {
    ...
    public func submit(_ configuration: PearchHANotarySubmissionConfiguration) throws -> PearchHANotarySubmissionResult {
        ...
        guard fileManager.fileExists(atPath: artifactURL.path) else {
            throw PearchHANotaryError.artifactMissing(artifactURL.path)
        }
        guard !keychainProfile.isEmpty else {
            throw PearchHANotaryError.blankKeychainProfile
        }
        guard configuration.waitForCompletion || !configuration.staple else {
            throw PearchHANotaryError.stapleRequiresCompletedSubmission
        }
        guard fileManager.isExecutableFile(atPath: xcrunURL.path) else {
            throw PearchHANotaryError.xcrunMissing(xcrunURL.path)
        }
        ...
        guard submitResult.status == 0 else {
            throw PearchHANotaryError.notaryToolFailed(status: submitResult.status, output: submitResult.output)
        }
        ...
        if configuration.staple {
            ...
            guard stapleResult.status == 0 else {
                throw PearchHANotaryError.staplerFailed(status: stapleResult.status, output: stapleResult.output)
            }
        }

        return PearchHANotarySubmissionResult(
...
public enum PearchHADMGBuildError: Error, Equatable, CustomStringConvertible, Sendable {
    case appBundleMissing(String)
    case outputExists(String)
...
public struct PearchHADMGBuildConfiguration: Equatable, Sendable {
    public let appURL: URL
    public let outputURL: URL
...
public struct PearchHADMGBuildResult: Equatable, Sendable {
    public let dmgURL: URL
    public let appURL: URL
    public let volumeName: String
}

public struct PearchHADMGBuilder {
    ...
    public func build(_ configuration: PearchHADMGBuildConfiguration) throws -> PearchHADMGBuildResult {
        let appURL = normalizedFileURL(configuration.appURL)
        ...
        guard !volumeName.isEmpty else {
            throw PearchHADMGBuildError.blankVolumeName
        }
        var isDirectory: ObjCBool = false
        guard fileManager.fileExists(atPath: appURL.path, isDirectory: &isDirectory), isDirectory.boolValue else {
            throw PearchHADMGBuildError.appBundleMissing(appURL.path)
        }
        if fileManager.fileExists(atPath: outputURL.path) {
            guard configuration.replaceExisting else {
                throw PearchHADMGBuildError.outputExists(outputURL.path)
            }
            try fileManager.removeItem(at: outputURL)
        }
        ...
        let stagingURL = outputURL.deletingLastPathComponent()
            .appendingPathComponent("pearchha-dmg-staging-\(UUID().uuidString)", isDirectory: true)
        defer {
        ...
        guard result.status == 0 else {
            throw PearchHADMGBuildError.hdiutilFailed(operation: "create", status: result.status, output: result.output)
        }
        guard fileManager.fileExists(atPath: outputURL.path) else {
            throw PearchHADMGBuildError.dmgMissing(outputURL.path)
        }
        return PearchHADMGBuildResult(dmgURL: outputURL, appURL: appURL, volumeName: volumeName)
    }
...
public struct PearchHADMGVerificationConfiguration: Equatable, Sendable {
    public let dmgURL: URL
...
public struct PearchHADMGVerificationResult: Equatable, Sendable {
    public let dmgURL: URL
}
...
public struct PearchHADMGVerifier {
    ...
    public init(
        fileManager: FileManager = .default,
        hdiutilURL: URL = PearchHADMGBuilder.defaultHdiutilURL,
        commandRunner: any PearchHACommandRunning = PearchHAFoundationCommandRunner()
    ) {
        self.fileManager = fileManager
        self.hdiutilURL = hdiutilURL
    ...
    public func verify(_ configuration: PearchHADMGVerificationConfiguration) throws -> PearchHADMGVerificationResult {
        let dmgURL = URL(fileURLWithPath: configuration.dmgURL.path)
        guard fileManager.fileExists(atPath: dmgURL.path) else {
            throw PearchHADMGBuildError.dmgMissing(dmgURL.path)
        }
        guard fileManager.isExecutableFile(atPath: hdiutilURL.path) else {
            throw PearchHADMGBuildError.hdiutilMissing(hdiutilURL.path)
        }
        ...
        return PearchHADMGVerificationResult(dmgURL: dmgURL)
    }
}

public struct PearchHADMGContentVerificationConfiguration: Equatable, Sendable {
    public let dmgURL: URL
    ...
    public init(
        dmgURL: URL,
        appBundleName: String = "PearchHA.app",
...
public struct PearchHADMGContentVerificationResult: Equatable, Sendable {
    public let dmgURL: URL
    public let mountedAppURL: URL
    public let applicationsShortcutURL: URL?
}

public struct PearchHADMGContentVerifier {
    private let fileManager: FileManager
    private let hdiutilURL: URL
    private let commandRunner: any PearchHACommandRunning
    ...
    public func verify(
        _ configuration: PearchHADMGContentVerificationConfiguration
    ) throws -> PearchHADMGContentVerificationResult {
        let dmgURL = URL(fileURLWithPath: configuration.dmgURL.path)
        guard fileManager.fileExists(atPath: dmgURL.path) else {
            throw PearchHADMGBuildError.dmgMissing(dmgURL.path)
        }
        ...
        let mountpointURL = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("pearchha-dmg-mount-\(UUID().uuidString)", isDirectory: true)
        ...
        guard attachResult.status == 0 else {
            throw PearchHADMGBuildError.hdiutilFailed(operation: "attach", status: attachResult.status, output: attachResult.output)
        }
        attached = true
        ...
        guard !appBundleName.isEmpty,
            ...
            throw PearchHADMGBuildError.mountedAppMissing(mountedAppURL.path)
        }
        ...
        if configuration.requireApplicationsShortcut {
            ...
            guard shortcutTarget == "/Applications" else {
                throw PearchHADMGBuildError.mountedApplicationsShortcutMissing(applicationsShortcutURL.path)
            }
        }
        ...
        guard detachResult.status == 0 else {
            throw PearchHADMGBuildError.hdiutilFailed(operation: "detach", status: detachResult.status, output: detachResult.output)
        }

        return PearchHADMGContentVerificationResult(
            dmgURL: dmgURL,
...
private struct PearchHAInfoPlistURLTypeSnapshot: Equatable, Sendable {
    let schemes: [String]
    let role: String
}

private struct PearchHAInfoPlist: Encodable {
    ...
    let urlTypes: [PearchHAInfoPlistURLType]
...
private struct PearchHAInfoPlistURLType: Encodable {
    let role = PearchHALaunchServicesVerifier.defaultURLTypeRole
    let name: String
    let schemes: [String]
