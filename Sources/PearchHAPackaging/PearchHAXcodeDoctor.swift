...
public enum PearchHAXcodePreflightIssue: Error, Equatable, CustomStringConvertible, Sendable {
...
public struct PearchHAXcodePreflightConfiguration: Equatable, Sendable {
...
public struct PearchHAXcodePreflightReport: Equatable, Sendable {
    ...
    public let issues: [PearchHAXcodePreflightIssue]
    ...
    public init(
        ...
        issues: [PearchHAXcodePreflightIssue]
    ) {
    ...
    ) -> PearchHAXcodePreflightDiagnostic {
        PearchHAXcodePreflightDiagnostic(report: self, configuration: configuration)
    }

    public func nextSteps(configuration: PearchHAXcodePreflightConfiguration) -> [String] {
        if isReadyForNativeVerification {
            return [
                "Full-Xcode native verification looks ready. Run the XCTest coverage gate and the PearchHA xcodebuild app check."
            ]
        }
    ...
    public func suggestedCommands(configuration: PearchHAXcodePreflightConfiguration) -> [String] {
        var commands: [String] = []
        if issues.contains(where: Self.isDeveloperDirectoryIssue) {
        ...
        if isReadyForNativeVerification {
            ...
            commands.append(
                prefixed(
                    "swift run pearchha-xcode-doctor --json --strict --project \(Self.shellQuoted(configuration.projectURL.path)) --scheme \(Self.shellQuoted(configuration.schemeName))"
                )
            )
        }
        return commands
    ...
    private static func isProjectMetadataIssue(_ issue: PearchHAXcodePreflightIssue) -> Bool {
        switch issue {
    ...
    private static func isDeveloperDirectoryIssue(_ issue: PearchHAXcodePreflightIssue) -> Bool {
        switch issue {
    ...
    private static func isDeveloperToolIssue(_ issue: PearchHAXcodePreflightIssue) -> Bool {
        switch issue {
...
public enum PearchHAXcodePreflightPresence: String, Codable, Equatable, Sendable {
    case present
    case missing
...
public enum PearchHAXcodePreflightState: String, Codable, Equatable, Sendable {
    case ready
    case blocked
...
public struct PearchHAXcodePreflightDiagnosticIssue: Codable, Equatable, Sendable {
...
public struct PearchHAXcodePreflightDiagnostic: Codable, Equatable, Sendable {
    public let project: PearchHAXcodePreflightPresence
    public let sharedScheme: PearchHAXcodePreflightPresence
    public let projectListing: PearchHAXcodePreflightState
    public let activeDeveloperDirectory: String?
    public let discoveredXcodeDeveloperDirectories: [String]
    public let fullXcode: PearchHAXcodePreflightState
    public let xctest: PearchHAXcodePreflightState
    public let xcodebuild: PearchHAXcodePreflightState
    public let nativeVerification: PearchHAXcodePreflightState
    public let issues: [PearchHAXcodePreflightDiagnosticIssue]
    public let nextSteps: [String]
    public let suggestedCommands: [String]

    public init(
        project: PearchHAXcodePreflightPresence,
        sharedScheme: PearchHAXcodePreflightPresence,
        projectListing: PearchHAXcodePreflightState,
        activeDeveloperDirectory: String?,
        discoveredXcodeDeveloperDirectories: [String],
        fullXcode: PearchHAXcodePreflightState,
        xctest: PearchHAXcodePreflightState,
        xcodebuild: PearchHAXcodePreflightState,
        nativeVerification: PearchHAXcodePreflightState,
        issues: [PearchHAXcodePreflightDiagnosticIssue],
        nextSteps: [String],
    ...
    public init(
        report: PearchHAXcodePreflightReport,
        configuration: PearchHAXcodePreflightConfiguration
    ) {
        project = PearchHAXcodePreflightPresence(report.projectAvailable)
        sharedScheme = PearchHAXcodePreflightPresence(report.sharedSchemeAvailable)
        projectListing = PearchHAXcodePreflightState(report.projectListingAvailable)
        ...
        fullXcode = PearchHAXcodePreflightState(report.fullXcodeSelected)
        xctest = PearchHAXcodePreflightState(report.xctestAvailable)
        xcodebuild = PearchHAXcodePreflightState(report.xcodebuildAvailable)
        nativeVerification = PearchHAXcodePreflightState(report.isReadyForNativeVerification)
        issues = report.issues.map {
            PearchHAXcodePreflightDiagnosticIssue(
...
public struct PearchHAXcodePreflightChecker {
    ...
    public func check(
        _ configuration: PearchHAXcodePreflightConfiguration
    ) throws -> PearchHAXcodePreflightReport {
        var issues: [PearchHAXcodePreflightIssue] = []
        ...
        return PearchHAXcodePreflightReport(
    ...
    private func activeDeveloperDirectory(
        issues: inout [PearchHAXcodePreflightIssue]
    ) throws -> String? {
    ...
    private func developerToolAvailable(
        _ tool: String,
        unavailableIssue: (Int32) -> PearchHAXcodePreflightIssue,
        issues: inout [PearchHAXcodePreflightIssue]
    ) throws -> Bool {
    ...
    private func appendLicenseIssueIfNeeded(issues: inout [PearchHAXcodePreflightIssue]) {
        if !issues.contains(.xcodeLicenseNotAccepted) {
...
public enum PearchHAXcodeDoctorCommand {
    ...
    public static func run(
        ...
        do {
            if arguments.contains("--help") || arguments.contains("-h") || arguments.first == "help" {
                standardOutput(PearchHAXcodeDoctorHelp.text)
                return 0
            }

            let options = try PearchHAXcodeDoctorOptions(arguments: arguments)
            let checker = PearchHAXcodePreflightChecker()
            ...
            standardError("pearchha-xcode-doctor: \(error)\n")
            return 1
        }
    }

    private static func render(diagnostic: PearchHAXcodePreflightDiagnostic) -> [String] {
        var lines = [
...
private enum PearchHAXcodeDoctorHelp {
    static let text = """
    Usage:
      pearchha-xcode-doctor [--project PATH] [--scheme NAME] [--json] [--strict]

    Options:
      --project PATH   Xcode project to verify. Default: PearchHA.xcodeproj
      --scheme NAME    Shared scheme name to verify. Default: PearchHA
      --json           Print machine-readable diagnostic JSON.
      --strict         Exit non-zero when native verification is blocked.
    """
}

private struct PearchHAXcodeDoctorOptions {
    let configuration: PearchHAXcodePreflightConfiguration
    let json: Bool
    let strict: Bool

    init(arguments: [String]) throws {
        let options: PearchHACommandLineOptions
        do {
            options = try PearchHACommandLineOptions(
                arguments: arguments,
                valueOptions: ["--project", "--scheme"],
                flagOptions: ["--json", "--strict"]
            )
        } catch let error as PearchHACommandLineParseError {
            throw PearchHAXcodeDoctorError.parseError(error)
        }

        let projectPath = options.value(for: "--project") ?? "PearchHA.xcodeproj"
        let schemeName = options.value(for: "--scheme") ?? "PearchHA"
        configuration = PearchHAXcodePreflightConfiguration(
...
private enum PearchHAXcodeDoctorError: Error, CustomStringConvertible {
    case parseError(PearchHACommandLineParseError)
