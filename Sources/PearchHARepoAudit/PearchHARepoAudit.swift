...
public enum PearchHARepoAuditCommand {
    ...
    public static func run(
        ...
        do {
            ...
            standardError("pearchha-repo-audit: \(error)\n")
            return 1
        }
    }
}
...
private struct GitFileList {
    ...
    func trackedAndUnignoredPaths() throws -> [String] {
        ...
        return Array(Set(parseNullSeparated(cached) + parseNullSeparated(unignored)))
            .filter {
                FileManager.default.fileExists(
                    atPath: rootURL.appendingPathComponent($0, isDirectory: false).path
                )
            }
            .sorted()
    }
