...
///
/// Only the kinds PearchHA actually emits at real state transitions are
/// represented. Each kind carries a calm, secret-free icon and label suitable
/// for the Diagnostics surface.
public enum PearchHADiagnosticEventKind: String, Equatable, Sendable, CaseIterable {
...
public struct PearchHADiagnosticEvent: Equatable, Identifiable, Sendable {
    public let id: UUID
    public let kind: PearchHADiagnosticEventKind
    public let message: String
    public let firstSeen: PearchInstant
    public let lastSeen: PearchInstant
    public let count: Int

    public init(
        id: UUID = UUID(),
        kind: PearchHADiagnosticEventKind,
        message: String,
        firstSeen: PearchInstant,
        lastSeen: PearchInstant,
        count: Int = 1
    ) {
        self.id = id
        self.kind = kind
    ...
    func collapses(withKind kind: PearchHADiagnosticEventKind, message: String) -> Bool {
...
/// into a snapshot.
public struct PearchHADiagnosticLog: Equatable, Sendable {
    /// The default maximum number of distinct events retained.
    public static let defaultCapacity = 50

    public let capacity: Int
    private var events: [PearchHADiagnosticEvent]

    public init(capacity: Int = PearchHADiagnosticLog.defaultCapacity, events: [PearchHADiagnosticEvent] = []) {
        self.capacity = max(1, capacity)
        self.events = Array(events.suffix(self.capacity))
    }

    /// The recorded events in oldest-first order.
    public var oldestFirst: [PearchHADiagnosticEvent] {
        events
    }

    /// The recorded events in newest-first order, for direct display.
    public var newestFirst: [PearchHADiagnosticEvent] {
        events.reversed()
    }
    ...
    public mutating func record(kind: PearchHADiagnosticEventKind, message: String, now: PearchInstant) {
        if let last = events.last, last.collapses(withKind: kind, message: message) {
            events[events.count - 1] = PearchHADiagnosticEvent(
                id: last.id,
                kind: last.kind,
        ...
        events.append(
            PearchHADiagnosticEvent(kind: kind, message: message, firstSeen: now, lastSeen: now)
        )
...
public enum PearchHARetryBackoffState: Equatable, Sendable {
