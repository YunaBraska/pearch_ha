...
public struct PerchHASectionHeader<Trailing: View>: View {
...
public extension PerchHASectionHeader where Trailing == EmptyView {
...
public struct PerchHAStateView: View {
    ...
    private var prominentBody: some View {
        VStack(spacing: PerchHASpacing.md) {
            Spacer(minLength: 0)
            symbol(size: 34)
            VStack(spacing: PerchHASpacing.xs) {
                Text(title)
                    .font(.headline)
                if let message {
                    Text(message)
        ...
        .padding(PerchHASpacing.xl)
        .accessibilityElement(children: .contain)
        .accessibilityLabel(accessibilityText)
    }

    private var inlineBody: some View {
        HStack(alignment: .top, spacing: PerchHASpacing.sm) {
            symbol(size: 13)
                .padding(.top, 1)
...
public struct PerchHALoadingState: View {
    private let title: String
    private let message: String?
...
public struct PerchHAEmptyState: View {
    private let systemImage: String
    private let title: String
    private let message: String?
    private let actionTitle: String?
...
public struct PerchHAErrorState: View {
    ...
    public var body: some View {
        PerchHAStateView(
            ...
            symbolTint: style == .inline ? PerchHATheme.critical : PerchHATheme.critical,
            title: title,
            message: message,
...
public struct PerchHAPermissionState: View {
    private let title: String
    private let message: String
    private let systemImage: String
...
///
/// Drives entirely off the passed ``PerchHADisplayPreferences`` and does no data
...
public struct PerchHAAppearancePreview: View {
    @Environment(\.colorScheme) private var colorScheme
    private let preferences: PerchHADisplayPreferences

    /// Creates the appearance preview.
    ///
    /// - Parameter preferences: The display preferences to visualize.
    public init(preferences: PerchHADisplayPreferences) {
    ...
    private var menuBarSample: some View {
        HStack(spacing: PerchHASpacing.sm) {
                    ...
                    Text("21°")
                        .font(
                            preferences.stableMenuBarWidth
                                ? PerchHATypography.bodyValue()
                                : PerchHATypography.body()
                        )
            ...
            .background(
                RoundedRectangle(cornerRadius: PerchHACornerRadius.control - 2, style: .continuous)
...
///
/// A pure mapping from ``PerchHAPanelPhase`` to the kind of state view the panel
...
public enum PerchHAStateViewKind: Equatable, Sendable {
    ...
    public static func forContent(phase: PerchHAPanelPhase) -> PerchHAStateViewKind {
        switch phase {
        case .connecting:
            .loading
        case .connectedEmpty:
