...
/// ``PearchHAEntityGauge`` fraction/severity rather than recomputing them.
public struct PearchHARingGauge: View {
...
public struct PearchHABarGauge: View {
    private let fraction: Double
    private let color: Color
    private let trackColor: Color
...
public struct PearchHABatteryGauge: View {
    private let fraction: Double
    private let color: Color
    private let trackColor: Color
...
/// Picks the drawn gauge family for a resolved ``PearchHAEntityGauge``.
///
...
public struct PearchHAEntityGaugeView: View {
    private let gauge: PearchHAEntityGauge

    public init(gauge: PearchHAEntityGauge) {
        self.gauge = gauge
    }

    public var body: some View {
        let color = PearchHATheme.color(for: gauge.severity)
        switch gauge.style {
        case .ring:
            PearchHARingGauge(fraction: gauge.fraction, color: color)
        case .bar:
            PearchHABarGauge(fraction: gauge.fraction, color: color)
        case .battery:
            PearchHABatteryGauge(fraction: gauge.fraction, color: color)
        }
    }
}
