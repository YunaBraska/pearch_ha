...
/// Dedicated, resizable Settings window content for PearchHA.
///
...
public struct PearchHASettingsView: View {
    ...
    private let model: PearchHAPanelModel
    @StateObject private var viewState: PearchHASettingsViewState
    private let accessibilityPreferencesOverride: PearchHAAccessibilityPreferences?
    private let displayPreferencesProvider: () -> PearchHADisplayPreferences
    private let displayPreferencesSink: (PearchHADisplayPreferences) -> Void
    ...
    @State private var displayPreferences: PearchHADisplayPreferences
    ...
    public init(
        ...
        accessibilityPreferencesOverride: PearchHAAccessibilityPreferences? = nil,
        ...
        self.model = model
        _viewState = StateObject(wrappedValue: PearchHASettingsViewState(model: model))
    ...
    private var snapshot: PearchHAPanelSnapshot {
        viewState.snapshot
    }
    ...
    private var oauthState: PearchHAOAuthSignInState {
        viewState.oauthSignInState
    }

    private var releaseState: PearchHAReleaseUpdateState {
        viewState.releaseUpdateState
    }

    private var retryState: PearchHARetryBackoffState {
        viewState.retryBackoffState
    }

    private var diagnosticsEvents: [PearchHADiagnosticEvent] {
        viewState.diagnosticEvents
    }

    public var body: some View {
        let accessibility = PearchHAPanelView.rootAccessibilityPresentation(
            snapshot: snapshot,
            preferences: accessibilityPreferences
        )
        let palette = PearchHATheme.Dashboard.palette(colorScheme)
    ...
    private var sidebarRail: some View {
        ScrollView(.vertical, showsIndicators: false) {
            ...
            .padding(.vertical, PearchHASpacing.md)
    ...
    private func sidebarRow(_ tab: Tab) -> some View {
        ...
        return Button {
            ...
            .padding(.horizontal, PearchHASpacing.sm)
            .padding(.vertical, PearchHASpacing.sm - 1)
            .frame(maxWidth: .infinity, alignment: .leading)
            .background(
                RoundedRectangle(cornerRadius: PearchHACornerRadius.control, style: .continuous)
            ...
            .overlay(alignment: .leading) {
                RoundedRectangle(cornerRadius: 2, style: .continuous)
                    ...
                    .offset(x: -PearchHASpacing.xs)
            }
            .contentShape(RoundedRectangle(cornerRadius: PearchHACornerRadius.control, style: .continuous))
        }
        .buttonStyle(.plain)
    ...
    private var railBackground: Color {
        PearchHATheme.Dashboard.palette(colorScheme).surfacePanel
    }
    ...
    private var accessibilityPreferences: PearchHAAccessibilityPreferences {
        if let accessibilityPreferencesOverride {
            return accessibilityPreferencesOverride
        }
        return PearchHAAccessibilityPreferences(
    ...
    public func tabContentForSnapshot(_ tab: Tab) -> some View {
        ...
        content
            .frame(maxWidth: .infinity, maxHeight: .infinity, alignment: .topLeading)
            .background(PearchHATheme.Dashboard.palette(colorScheme).surfaceRoot.ignoresSafeArea())
            .environment(\.dashboardPalette, PearchHATheme.Dashboard.palette(colorScheme))
    ...
    private var connectionStatusContent: some View {
        ...
        return VStack(alignment: .leading, spacing: PearchHASpacing.sm) {
            HStack(spacing: 8) {
                StatusPill(
                    status.label,
    ...
    private func settingsPage<Content: View>(
        ...
        let palette = PearchHATheme.Dashboard.palette(colorScheme)
        ScrollView(.vertical) {
            VStack(alignment: .leading, spacing: PearchHASpacing.md + 2) {
                Text(title)
            ...
            .frame(maxWidth: .infinity, alignment: .leading)
            .padding(.vertical, PearchHASpacing.lg)
            .padding(.horizontal, PearchHASpacing.lg + 2)
        }
        .toggleStyle(PearchHASwitchToggleStyle())
        .tint(PearchHATheme.accent)
        .frame(maxWidth: .infinity, maxHeight: .infinity, alignment: .topLeading)
    }
    ...
    private func settingsCard<Content: View>(@ViewBuilder content: () -> Content) -> some View {
        let palette = PearchHATheme.Dashboard.palette(colorScheme)
        return VStack(alignment: .leading, spacing: PearchHASpacing.md) {
            content()
        }
        .padding(PearchHASpacing.lg)
        .frame(maxWidth: .infinity, alignment: .leading)
        .background(
            RoundedRectangle(cornerRadius: PearchHACornerRadius.card, style: .continuous)
        ...
        settingsPage(title: "Appearance", systemImage: Tab.appearance.systemImage) {
            settingsCard {
                settingsSection(title: "Preview", systemImage: "eye") {
                    VStack(alignment: .leading, spacing: PearchHASpacing.md) {
                        PearchHAAppearancePreview(preferences: displayPreferences)
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        settingsControlRow("Style") {
                            Picker("Menu bar style", selection: menuBarAppearanceBinding) {
                                ForEach(PearchHAMenuBarAppearance.allCases, id: \.rawValue) { appearance in
                    ...
                    VStack(alignment: .leading, spacing: PearchHASpacing.sm + 2) {
                        settingsControlRow("Row density") {
                            Picker("Row density", selection: rowDensityBinding) {
                                ForEach(PearchHADashboardRowDensity.allCases, id: \.rawValue) { density in
    ...
    /// both a menu-bar item (from ``PearchHAAppearancePreview``) and a real
    ...
    private var themeModePicker: some View {
        Picker("Theme", selection: themeModeBinding) {
            ForEach(PearchHAThemeMode.allCases, id: \.rawValue) { mode in
    ...
    private var accentMenuPicker: some View {
        Picker("Accent", selection: accentSwatchBinding) {
            ForEach(PearchHAAccentColor.swatches) { swatch in
                Text(swatch.name).tag(swatch.id)
            }
        }
        .labelsHidden()
        .frame(width: 160)
        .pearchHACompactControl()
    ...
    private func color(for accent: PearchHAAccentColor) -> Color {
        ...
        return VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 8) {
                StatusPill(
                    ...
                    color: isHealthy ? PearchHATheme.ok : PearchHATheme.warn,
    ...
    private func diagnosticEventRow(_ event: PearchHADiagnosticEvent) -> some View {
        let tint: Color = {
            switch event.kind {
            case .recovered: return PearchHATheme.ok
            case .reconnecting, .liveUpdatesInterrupted: return PearchHATheme.warn
            case .connectionFailed, .refreshFailed: return PearchHATheme.critical
            }
        }()
        ...
        return HStack(alignment: .firstTextBaseline, spacing: 8) {
            ...
            if event.count > 1 {
                Text("×\(event.count)")
                    .font(PearchHATypography.caption().weight(.semibold))
                    .foregroundStyle(.secondary)
            }
                    ...
                    ForEach(Array(health.visibleGroups.enumerated()), id: \.offset) { index, group in
                        HStack(spacing: 8) {
                            Image(systemName: group.systemImage)
                                .font(.caption)
                                .foregroundStyle(PearchHATheme.warn)
                                .frame(width: 18)
                                .accessibilityHidden(true)
                            Text(group.label)
                                .font(.callout)
    ...
    private var connectionDiagnostic: (label: String, detail: String, systemImage: String, color: Color) {
        ...
        case let .reconnecting(attempt):
            return ("Reconnecting", "Attempt \(attempt). Showing the last known values.", "arrow.triangle.2.circlepath", PearchHATheme.warn)
        case .disconnected:
            return ("Disconnected", "Not connected to Home Assistant.", "bolt.horizontal.circle", .secondary)
        case let .failed(failure):
            switch failure {
            case .authentication:
                return ("Permission", "Sign in again or check your access token.", "lock.circle.fill", PearchHATheme.critical)
            case .unreachable, .tlsRejected, .unsupportedCommand, .protocolError:
                return ("Error", "Connection failed. See the last error below.", "exclamationmark.circle.fill", PearchHATheme.critical)
            }
        }
    }
    ...
    private func updateDisplayPreferences(_ preferences: PearchHADisplayPreferences) {
    ...
    private var menuBarAppearanceBinding: Binding<PearchHAMenuBarAppearance> {
        Binding(
    ...
    private func refreshIntervalIndexBinding(
        get currentValue: @escaping () -> PearchHAMenuBarRefreshInterval,
        set apply: @escaping (PearchHAMenuBarRefreshInterval) -> Void
    ) -> Binding<Double> {
        Binding(
            get: {
                Double(
    ...
    private var rowDensityBinding: Binding<PearchHADashboardRowDensity> {
        Binding(
    ...
    private var themeModeBinding: Binding<PearchHAThemeMode> {
        Binding(
    ...
    private var accentSwatchBinding: Binding<String> {
        Binding(
            get: { displayPreferences.accentColor.matchingSwatchID ?? PearchHAAccentColor.swatches.first?.id ?? "ha-blue" },
            set: { id in
                guard let swatch = PearchHAAccentColor.swatches.first(where: { $0.id == id }) else {
                    return
                }
                updateDisplayPreferences(displayPreferences.with(accentColor: swatch.color))
            }
    ...
    private func aboutSupportLink(_ title: String, systemImage: String, destination: URL) -> some View {
        ...
        .buttonStyle(PearchHAIconButtonStyle(prominentOnHover: true))
    ...
    private static let applicationVersion = PearchHAApplicationVersionInfo.currentBundle()
    private static let refreshIntervalOptions = PearchHAMenuBarRefreshInterval.allCases
    ...
    private var settingsContent: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 8) {
                PearchHACapsuleField(systemImage: "magnifyingglass") {
        ...
        VStack(alignment: .leading, spacing: 5) {
            ...
            ForEach(model.orphanedCustomActions, id: \.id.rawValue) { action in
                HStack(spacing: 6) {
                    ...
                    .buttonStyle(PearchHACircularIconButtonStyle())
                    .controlSize(.small)
    ...
    private func withSelectionAnimation(_ body: () -> Void) {
        ...
        if let animation = PearchHAMotion.animation(reduceMotion: reduceMotion) {
    ...
    private func selectionEntityRowSegment(
        ...
        .padding(.top, isFirst ? PearchHASpacing.xs : 0)
        .padding(.bottom, isLast ? PearchHASpacing.xs : 0)
        .background(
            SelectionRoomSegmentShape(
                radius: PearchHACornerRadius.card,
                roundsTop: isFirst,
                roundsBottom: isLast
            )
            .fill(palette.surfacePanel)
    ...
    private func selectionRoomHeader(
        ...
        return HStack(spacing: PearchHASpacing.xs + 1) {
            Button {
    ...
    private func selectionEntityRow(
        ...
        let rowCaption = PearchHAEntityMetadataPresentation.rowCaption(for: entity)
        ...
        return VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 10) {
                ...
                Image(systemName: pearchHAEntityIconName(for: entity))
                    .font(.system(size: 14))
    ...
    private var menuBarVisiblePill: some View {
        collapsedEntityStatusPill(
            ...
            color: PearchHATheme.Dashboard.palette(colorScheme).accentPrimary,
    ...
    private var averageLinkedPill: some View {
        collapsedEntityStatusPill(
            systemImage: "link",
            color: PearchHATheme.Dashboard.palette(colorScheme).warning,
    ...
    private func entityDetailSections(for entity: DiscoveredEntity, roomName: String) -> some View {
        ...
        let links = PearchHAEntityMetadataLinksPresentation(
    ...
    private func entityIdentitySection(
        _ fields: [PearchHAEntityMetadataPresentation.Field],
        links: PearchHAEntityMetadataLinksPresentation
    ) -> some View {
    ...
    @ViewBuilder
    private func metadataFieldValueText(_ field: PearchHAEntityMetadataPresentation.Field) -> some View {
        Text(field.value)
            .font(
                field.usesMonospacedFont
                    ? .system(size: 11.5, weight: .regular, design: .monospaced)
                    : PearchHATypography.caption()
            )
    ...
    private func metadataLink(
        for field: PearchHAEntityMetadataPresentation.Field,
        links: PearchHAEntityMetadataLinksPresentation
    ) -> URL? {
        switch field.label {
        case "Entity ID":
    ...
    private func accessibilityLabel(for field: PearchHAEntityMetadataPresentation.Field) -> String {
        switch field.label {
        case "Entity ID":
    ...
    /// Symbol header and right-aligned controls in the bespoke PearchHA language.
    private func settingsSection<Content: View>(
        ...
        VStack(alignment: .leading, spacing: PearchHASpacing.sm) {
            PearchHASectionHeader(title, systemImage: systemImage)
            content()
                .frame(maxWidth: .infinity, alignment: .leading)
        }
        .frame(maxWidth: .infinity, alignment: .leading)
    ...
    private func entityIconPicker(
        ...
        Menu {
            Button {
                ...
                Label("Automatic", systemImage: pearchHAEntityIconName(for: entity))
            }
            ForEach(PearchHAEntityIconCatalog.sections, id: \.title) { section in
            ...
            Label(
                ...
                systemImage: configuration.customIconName ?? pearchHAEntityIconName(for: entity)
            )
        }
        .fixedSize()
    ...
    private func semanticThresholdSwatch(_ accent: PearchHAAccentColor) -> some View {
        RoundedRectangle(cornerRadius: 6, style: .continuous)
            .fill(PearchHATheme.color(for: accent))
    ...
    private func thresholdStepColorBinding(
        ...
        Binding(
            get: {
                PearchHATheme.color(for: step.color)
            },
            set: { newColor in
    ...
    private func thresholdBaseColorBinding(
        ...
        Binding(
            get: {
                thresholds.baseColor.map(PearchHATheme.color(for:)) ?? PearchHATheme.accent
            },
            set: { newColor in
                guard let accent = accentColor(from: newColor) else {
                    return
    ...
    private func stateThresholdRuleColorBinding(
        ...
        Binding(
            get: {
                PearchHATheme.color(for: rule.color)
            },
            set: { newColor in
                guard let accent = accentColor(from: newColor) else {
                    return
    ...
    private func accentColor(from color: Color) -> PearchHAAccentColor? {
        PearchHAAccentColor(color)
    }
    ...
    private func setThresholdBaseColor(
        _ baseColor: PearchHAAccentColor?,
        for id: EntityID,
        thresholds: ValueThresholds
    ) {
    ...
    private func menuBarItemAppearanceBinding(for id: EntityID) -> Binding<PearchHAMenuBarAppearance> {
        Binding(
            get: {
        ...
        Binding(
            get: {
                menuBarItemConfiguration(for: id).minValue
                    .map { PearchHABoundsField.text(for: $0) } ?? ""
            },
            set: { text in
                let configuration = menuBarItemConfiguration(for: id)
                model.setDisplayBounds(id, minValue: PearchHABoundsField.value(from: text), maxValue: configuration.maxValue)
            }
        )
    }
    ...
    private func displayMaxBinding(for id: EntityID) -> Binding<String> {
        Binding(
            ...
            set: { text in
                let configuration = menuBarItemConfiguration(for: id)
                model.setDisplayBounds(id, minValue: configuration.minValue, maxValue: PearchHABoundsField.value(from: text))
            }
        )
    }
...
private enum PearchHABoundsField {
...
public enum PearchHAEntityIconCatalog {
