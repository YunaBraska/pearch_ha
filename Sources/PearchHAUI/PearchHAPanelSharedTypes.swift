...
extension ActionValue {
    var editorType: PearchHACustomActionServiceDataValueKind {
        switch self {
        case .number:
            .number
        case .bool:
...
public struct PearchHAHistoryPopoverContent: View {
    ...
    private var palette: PearchHATheme.DashboardPalette {
        PearchHATheme.Dashboard.palette(colorScheme)
    }
    ...
    public var body: some View {
        ...
        return VStack(alignment: .leading, spacing: 12) {
            ...
            PearchHAHistoryRangeSegmentedControl(
    ...
    private var historyBody: some View {
        switch PearchHAHistoryBodyPresentation(state: state, entityID: entityID) {
        case .hidden:
            EmptyView()
        ...
        case let .stateTimeline(series, segments):
            ...
            PearchHAHistoryStateTimelinePopoverBody(
                segments: segments,
                range: series.range,
    ...
    private func cursorReadout(for series: HistorySeries) -> String? {
        guard let normalizedX = cursorNormalizedX,
              let sample = PearchHAHistoryCursor.nearestSample(in: series, atNormalizedX: normalizedX)
        else {
            return nil
        }
    ...
    static func cursorReadout(
        ...
        let formattedTimestamp = PearchHAHistoryHoverFormatting.timestamp(
            timestamp,
            range: range,
            locale: locale,
            timeZone: timeZone
    ...
    private func historyStats(_ statistics: PearchHAHistoryStatistics) -> some View {
...
private struct PearchHAHistoryRangeSegmentedControl: NSViewRepresentable {
...
enum PearchHASliderValueFormatting {
...
public func pearchHAEntityIconName(for entity: DiscoveredEntity) -> String {
...
@MainActor
final class PearchHAMenuActionTarget: NSObject {
...
extension PearchHAPanelSnapshot {
...
@MainActor
struct PearchHANativeTextField: NSViewRepresentable {
    ...
    final class Coordinator: NSObject, NSTextFieldDelegate {
        var parent: PearchHANativeTextField

        init(parent: PearchHANativeTextField) {
            self.parent = parent
        }
...
@MainActor
struct PearchHANativeSecureField: NSViewRepresentable {
    let placeholder: String
    @Binding var text: String
    let contentType: NSTextContentType?
    ...
    @MainActor
    final class Coordinator: NSObject, NSTextFieldDelegate {
        var parent: PearchHANativeSecureField

        init(parent: PearchHANativeSecureField) {
            self.parent = parent
        }
...
struct PearchHACoverPositionSlider: View {
    let position: Int
    let disabled: Bool
    ...
    var body: some View {
        HStack(spacing: PearchHASpacing.sm) {
            ...
            .accessibilityValue(PearchHASliderValueFormatting.accessibilityValue(for: draft, unit: unit))
            .onChange(of: position) { newPosition in
                if !isEditing {
                    draft = Double(newPosition)
                }
            }
            Text(PearchHASliderValueFormatting.label(for: draft, unit: unit))
...
public typealias PearchHABootstrapView = PearchHAPanelView

public enum PearchHAUI {
