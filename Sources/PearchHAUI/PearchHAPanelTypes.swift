...
public struct PearchHAConnectionAddressField: Equatable, Identifiable, Sendable {
    public let id: UUID
    ...
    public var validURL: URL? {
        ...
        return PearchHAConnectionForm.validURL(urlString)
    }
}

public struct PearchHAConnectionForm: Equatable, Sendable {
    public var urlString: String
    /// Ordered alternative addresses tried after the primary ``urlString``.
    public var addresses: [PearchHAConnectionAddressField]
    ...
    public init(
        urlString: String = "",
        addresses: [PearchHAConnectionAddressField] = [],
        token: String = "",
    ...
    public init(
        ...
        self.init(
            urlString: urlString,
            addresses: trimmedFallback.isEmpty ? [] : [PearchHAConnectionAddressField(urlString: fallbackURLString)],
            token: token,
    ...
    public func sameConnection(as other: PearchHAConnectionForm?) -> Bool {
...
public enum PearchHAConnectionAttemptResult: Equatable, Sendable {
    case success(rooms: [Room])
    case failure(ConnectionFailure)
}

public enum PearchHAPanelPhase: Equatable, Sendable {
    case firstRun
    case connecting
    case connectedEmpty
    case connectedData
...
public enum PearchHAHistoryProviderResult: Equatable, Sendable {
    case success(HistorySeries)
    case unavailable(String)
}

public enum PearchHAActionResult: Equatable, Sendable {
    case success
    case failed(String)
}

public enum PearchHAServiceMetadataProviderResult: Equatable, Sendable {
    case success([HAServiceMetadata])
    case unavailable(String)
}

public enum PearchHAOAuthSignInResult: Equatable, Sendable {
    case success
    case failed(String)
}

public enum PearchHAOAuthSignInState: Equatable, Sendable {
    case idle
    case signingIn
    case failed(String)
}

public struct PearchHAReleaseUpdate: Equatable, Sendable {
...
public enum PearchHAReleaseUpdateCheckResult: Equatable, Sendable {
    case upToDate(currentVersion: String, latestVersion: String, releaseURL: URL)
    case updateAvailable(PearchHAReleaseUpdate)
    case failed(String)
}

public enum PearchHAReleaseUpdateState: Equatable, Sendable {
    case idle
    case checking
    case upToDate(currentVersion: String, latestVersion: String, releaseURL: URL)
    case updateAvailable(PearchHAReleaseUpdate)
...
public enum PearchHACustomActionServiceDataValueKind: String, CaseIterable, Hashable, Sendable {
    case string
    case number
    case bool
    case object
...
public enum PearchHACustomActionServiceDataPathComponent: Equatable, Hashable, Sendable {
    case key(String)
    case index(Int)
}

public enum PearchHAControlActionState: Equatable, Sendable {
    case idle
...
public struct PearchHAEntityControl: Equatable, Sendable {
    public let entityID: EntityID
    public let isOn: Bool
    public let isRunning: Bool
    public let failureMessage: String?

    public init?(entity: DiscoveredEntity, actionState: PearchHAControlActionState = .idle) {
...
public enum PearchHACoverCommand: Equatable, Sendable {
    case open
    case close
    case stop
...
public struct PearchHACoverControl: Equatable, Sendable {
    public let entityID: EntityID
    ...
    public func actionSpec(command: PearchHACoverCommand) -> ActionSpec {
    ...
    public func optimisticState(command: PearchHACoverCommand, currentState: String) -> String {
        switch command {
        case .open:
            "open"
        case .close:
    ...
    public func optimisticPosition(command: PearchHACoverCommand) -> Int? {
        switch command {
        case .open:
...
public enum PearchHAHistoryPanelState: Equatable, Sendable {
    case idle
...
public struct PearchHAHistoryStatistics: Equatable, Sendable {
...
public enum PearchHAHistoryContentSummary: Equatable, Sendable {
    ...
    case statistics(PearchHAHistoryStatistics)
    ...
    private static func weightedAverage(_ samples: [PearchHAHistoryCursorSample]) -> Double {
...
public enum PearchHAHistoryBodyPresentation: Equatable, Sendable {
    case hidden
    ...
    case statistics(series: HistorySeries, statistics: PearchHAHistoryStatistics)
    case unavailable(String)

    public init(state: PearchHAHistoryPanelState, entityID: EntityID) {
        switch state {
        case .idle:
            self = .hidden
        ...
        case let .loaded(series) where series.entityID == entityID:
            switch PearchHAHistoryContentSummary(series: series) {
            case .empty:
                self = .empty
            case .noNumericData:
                self = .noNumericData
...
public struct PearchHAHistorySparklinePoint: Equatable, Sendable {
    public let x: Double
    public let y: Double
...
public struct PearchHAHistorySparklineGeometry: Equatable, Sendable {
    ...
    public init(samples: [PearchHAHistoryCursorSample], maxSamples: Int? = nil) {
        ...
        points = samples.enumerated().map { index, sample in
            ...
            let y = valueSpan > 0
                ...
                : 0.5
            return PearchHAHistorySparklinePoint(
                x: x.clamped(to: 0...1),
                y: y.clamped(to: 0...1)
            )
        }
    ...
    private static func downsample(
        _ samples: [PearchHAHistoryCursorSample],
        to maxSamples: Int?
    ) -> [PearchHAHistoryCursorSample] {
        guard let maxSamples, maxSamples >= 2, samples.count > maxSamples else {
            return samples
        }
        let lastIndex = samples.count - 1
        var picked: [PearchHAHistoryCursorSample] = []
...
public enum PearchHAAccessibilityMotionPolicy: String, Equatable, Sendable {
    case system
    case reduced
}

public enum PearchHAAccessibilityContrastPolicy: String, Equatable, Sendable {
    case standard
    case increased
}

public struct PearchHAAccessibilityPreferences: Equatable, Sendable {
    ...
    public var motionPolicy: PearchHAAccessibilityMotionPolicy {
        reduceMotion ? .reduced : .system
    }

    public var contrastPolicy: PearchHAAccessibilityContrastPolicy {
        increaseContrast ? .increased : .standard
    }
}

public struct PearchHAPanelAccessibilityPresentation: Equatable, Sendable {
    public let summary: String
    public let statusLabel: String
    public let contentLabel: String
    public let keyboardHint: String
    public let motionPolicy: PearchHAAccessibilityMotionPolicy
    public let contrastPolicy: PearchHAAccessibilityContrastPolicy
    ...
    public init(
        ...
        contentLabel: String,
        keyboardHint: String,
        motionPolicy: PearchHAAccessibilityMotionPolicy,
        contrastPolicy: PearchHAAccessibilityContrastPolicy
    ) {
...
public struct PearchHAControlAccessibilityPresentation: Equatable, Sendable {
...
public struct PearchHAEntityRowAccessibilityPresentation: Equatable, Sendable {
    public let label: String
    public let value: String
    public let controls: [PearchHAControlAccessibilityPresentation]
    public let failureLabel: String?

    public init(
        label: String,
        value: String,
        controls: [PearchHAControlAccessibilityPresentation],
...
public struct PearchHAPanelRootAccessibilityPresentation: Equatable, Sendable {
    public let label: String
    public let value: String
    public let hint: String
    public let motionPolicy: PearchHAAccessibilityMotionPolicy
    ...
    public init(panel: PearchHAPanelAccessibilityPresentation) {
...
public struct PearchHAPanelEntityContextPresentation: Equatable, Sendable {
...
extension HistorySeries {
    var chronologicalNumericSamples: [PearchHAHistoryCursorSample] {
        PearchHAHistoryCursor.numericSamples(of: self)
    }
}

public struct PearchHAHistoryCacheConfiguration: Equatable, Sendable {
    public let capacity: Int
    public let ttl: PearchDuration
    ...
    public init(capacity: Int = 256, ttl: PearchDuration = .seconds(60)) {
...
public struct PearchHAPeriodicRefreshConfiguration: Equatable, Sendable {
    /// The base interval between refreshes while the panel is active.
    public let interval: PearchDuration
    /// The maximum backoff interval applied after consecutive failures.
    public let maximumBackoff: PearchDuration
    ...
    public init(
        interval: PearchDuration = .seconds(45),
        maximumBackoff: PearchDuration = .seconds(300)
    ) {
    ...
    public static let disabled = PearchHAPeriodicRefreshConfiguration(interval: .seconds(0))
}
...
/// events flow again.
public struct PearchHALiveUpdateConfiguration: Equatable, Sendable {
    /// The delay before the first reconnect attempt after a dropped stream.
    public let reconnectDelay: PearchDuration
    ...
    public init(
        reconnectDelay: PearchDuration = .seconds(1),
        maximumBackoff: PearchDuration = .seconds(60)
    ) {
    ...
    public static let disabled = PearchHALiveUpdateConfiguration(reconnectDelay: .seconds(0))
}
...
public struct PearchHAHistoryBulkSyncConfiguration: Equatable, Sendable {
    ...
    public let settleDelay: PearchDuration
    ...
    public init(
        interval: PearchDuration = .seconds(30),
        settleDelay: PearchDuration = .milliseconds(250),
        batchSize: Int = 40,
    ...
    public static let disabled = PearchHAHistoryBulkSyncConfiguration(interval: .seconds(0))
}

public struct PearchHAPanelSnapshot: Equatable, Sendable {
    public let connectionState: ConnectionState
    public let phase: PearchHAPanelPhase
    ...
    public init(
        ...
        phase: PearchHAPanelPhase = .firstRun,
        rooms: [Room] = [],
        availableRooms: [Room] = [],
        ...
        connectionForm: PearchHAConnectionForm = PearchHAConnectionForm(),
        ...
        historyState: PearchHAHistoryPanelState = .idle,
        historyPresentationEntityID: EntityID? = nil,
        controlActionState: PearchHAControlActionState = .idle,
    ...
    struct Draft {
        ...
        var connectionForm: PearchHAConnectionForm
        ...
        var historyState: PearchHAHistoryPanelState
        var historyPresentationEntityID: EntityID?
        var controlActionState: PearchHAControlActionState
        var serviceMetadata: [HAServiceMetadata]

        init(snapshot: PearchHAPanelSnapshot) {
        ...
        func snapshot() -> PearchHAPanelSnapshot {
    ...
    func updating(_ update: (inout Draft) -> Void) -> PearchHAPanelSnapshot {
    ...
    public func control(for entity: DiscoveredEntity) -> PearchHAEntityControl? {
        PearchHAEntityControl(entity: entity, actionState: controlActionState)
    }

    public func coverControl(for entity: DiscoveredEntity) -> PearchHACoverControl? {
        PearchHACoverControl(entity: entity, actionState: controlActionState)
    }
    ...
    public func dashboardSummary(
        ...
        return PearchHADashboardSummary(
            connectionState: connectionState,
    ...
    private func resolveSelectedMetrics(
        ...
        locale: Locale
    ) -> [PearchHADashboardSummary.SelectedMetric] {
        let capped = PearchHADisplayPreferences.cappedSummaryMetricEntityIDs(selectedMetricIDs)
        return capped.map { id in
            guard let entity = allEntities.first(where: { $0.id == id }) else {
                return PearchHADashboardSummary.SelectedMetric(
                    entityID: id,
                    name: id.rawValue,
                    valueText: "—",
                    isAvailable: false
    ...
    private func resolvePrimaryMetric(
        allEntities: [DiscoveredEntity],
        locale: Locale
    ) -> PearchHADashboardSummary.PrimaryMetric? {
        ...
        for entity in promotedFirst + allEntities {
            ...
            case let .gauge(gauge):
                return PearchHADashboardSummary.PrimaryMetric(
                    entityID: entity.id,
                    name: entity.name,
    ...
    /// The live-effective per-entity display configuration.
    ...
    private func displayedEntity(for entity: DiscoveredEntity) -> DiscoveredEntity {
        PearchHAEntityAveraging.averagedEntity(
            base: entity,
            configuration: effectiveMenuBarItemConfiguration(for: entity),
    ...
    public func accessibilityPresentation(
        preferences: PearchHAAccessibilityPreferences = PearchHAAccessibilityPreferences()
    ) -> PearchHAPanelAccessibilityPresentation {
        PearchHAPanelAccessibilityPresentation(
    ...
    public func accessibilityRow(
    ...
    ) -> PearchHAEntityRowAccessibilityPresentation {
        let value = formattedValue(for: entity)
        var controls: [PearchHAControlAccessibilityPresentation] = []
        ...
        return PearchHAEntityRowAccessibilityPresentation(
    ...
    public func redactedForDiagnostics() -> PearchHAPanelSnapshot {
        PearchHAPanelSnapshot(
            connectionState: connectionState,
            phase: phase,
            rooms: rooms,
    ...
    private func toggleAccessibility(
        ...
        return PearchHAControlAccessibilityPresentation(
...
private struct HistoryRowKeyboardAccess: ViewModifier {
    ...
    func body(content: Content) -> some View {
        if #available(macOS 14.0, *) {
            content
                .background(
                    (isPresented || isFocused) ? PearchHATheme.accent.opacity(0.08) : Color.clear
                )
                .focusable()
                .focused($isFocused)
                .focusEffectDisabled()
...
actor PearchHALiveEventReceipt {
...
struct PearchHAHistoryCacheKey: Hashable, Sendable {
    let entityID: EntityID
    let range: HistoryRange
}

private struct PearchHAHistoryCacheEntry {
    let series: HistorySeries
    let expiresAt: PearchInstant
}

struct PearchHAHistoryCache {
    private var entries: [PearchHAHistoryCacheKey: PearchHAHistoryCacheEntry] = [:]
    private var order: [PearchHAHistoryCacheKey] = []
    ...
    mutating func series(for key: PearchHAHistoryCacheKey, now: PearchInstant) -> HistorySeries? {
    ...
    func peekAllowingStale(for key: PearchHAHistoryCacheKey) -> HistorySeries? {
        entries[key]?.series
    }

    mutating func insert(
        _ series: HistorySeries,
        for key: PearchHAHistoryCacheKey,
        now: PearchInstant,
        capacity: Int,
        ttl: PearchDuration,
        protecting protected: Set<PearchHAHistoryCacheKey> = []
    ) {
        entries[key] = PearchHAHistoryCacheEntry(
            series: series,
    ...
    private mutating func markRecentlyUsed(_ key: PearchHAHistoryCacheKey) {
    ...
    private mutating func trim(to capacity: Int, protecting protected: Set<PearchHAHistoryCacheKey>) {
...
enum PearchHAInlineHistoryPreviewData {
    case sparkline(PearchHAHistorySparklineGeometry)
