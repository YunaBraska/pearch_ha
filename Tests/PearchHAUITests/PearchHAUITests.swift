...
import PearchHAAppShell
import PearchHAPersistence
import PearchHASupport
import PearchHATestSupport
@testable import PearchHAUI

@MainActor
final class PearchHAUITests: XCTestCase {
    func testPanelSnapshotExposesConnectionStateSummary() {
        let snapshot = PearchHAPanelSnapshot(connectionState: .reconnecting(attempt: 2))
    ...
    func testSettingsEntityMetadataPresentationSurfacesIdentityFields() {
        let presentation = PearchHAEntityMetadataPresentation(
    ...
    func testSettingsEntityMetadataLinksPresentationBuildsEntityAndDeviceURLs() {
        let presentation = PearchHAEntityMetadataLinksPresentation(
    ...
    func test_t_accessibility_summary_reflects_empty_loading_success_and_error_states() {
        let firstRun = PearchHAPanelSnapshot()
        let connecting = PearchHAPanelSnapshot(connectionState: .connecting, phase: .connecting)
        let empty = PearchHAPanelSnapshot(connectionState: .connected, phase: .connectedEmpty)
        let rooms = selectionRooms()
        let success = PearchHAPanelSnapshot(
            connectionState: .connected,
            phase: .connectedData,
            rooms: rooms,
            availableRooms: rooms
        )
        let failed = PearchHAPanelSnapshot(
            connectionState: .failed(.authentication),
            phase: .failed(.authentication)
        )
        let stale = PearchHAPanelSnapshot(
    ...
    func testPanelEntityContextPresentationReflectsMenuBarSettings() {
        ...
        let presentation = PearchHAPanelView.entityContextPresentation(
            snapshot: snapshot,
            entityID: entityID
        )
            ...
            XCTAssertEqual(
            presentation,
            PearchHAPanelEntityContextPresentation(
    ...
    func test_t_accessibility_preferences_map_reduce_motion_and_increase_contrast() {
        ...
        let increased = PearchHAPanelSnapshot().accessibilityPresentation(
            preferences: PearchHAAccessibilityPreferences(reduceMotion: true, increaseContrast: true)
        )
    ...
    func test_t_design_tokens_expose_the_documented_spacing_and_radius_scale() {
        XCTAssertEqual(PearchHASpacing.xs, 4)
        XCTAssertEqual(PearchHASpacing.sm, 8)
        XCTAssertEqual(PearchHASpacing.md, 12)
        XCTAssertEqual(PearchHASpacing.lg, 16)
        XCTAssertEqual(PearchHASpacing.xl, 24)
        XCTAssertEqual(PearchHACornerRadius.control, 8)
        XCTAssertEqual(PearchHACornerRadius.card, 12)
        XCTAssertEqual(PearchHACornerRadius.panel, 16)
    }

    func test_t_motion_token_duration_sits_in_the_documented_range() {
        XCTAssertEqual(PearchHAMotion.standardDuration, 0.18, accuracy: 0.0001)
        XCTAssertGreaterThanOrEqual(PearchHAMotion.standardDuration, 0.12)
        XCTAssertLessThanOrEqual(PearchHAMotion.standardDuration, 0.20)
    }

    func test_t_motion_animation_helper_is_nil_only_when_reduce_motion_is_on() {
        XCTAssertNil(PearchHAMotion.animation(reduceMotion: true))
        XCTAssertNotNil(PearchHAMotion.animation(reduceMotion: false))
    }

    func test_t_state_view_kind_maps_loading_phase() {
        XCTAssertEqual(PearchHAStateViewKind.forContent(phase: .connecting), .loading)
    }

    func test_t_state_view_kind_maps_empty_phase() {
        XCTAssertEqual(PearchHAStateViewKind.forContent(phase: .connectedEmpty), .empty)
    }

    func test_t_state_view_kind_maps_connection_form_phases() {
        XCTAssertEqual(PearchHAStateViewKind.forContent(phase: .firstRun), .connectionForm)
        XCTAssertEqual(
            PearchHAStateViewKind.forContent(phase: .failed(.authentication)),
            .connectionForm
        )
    }

    func test_t_state_view_kind_maps_data_phases() {
        XCTAssertEqual(PearchHAStateViewKind.forContent(phase: .connectedData), .data)
        XCTAssertEqual(PearchHAStateViewKind.forContent(phase: .reconnecting(attempt: 1)), .data)
        XCTAssertEqual(
            PearchHAStateViewKind.forContent(phase: .failedStale(.unreachable(host: "ha.local"))),
            .data
        )
    }
    ...
    func test_t_accessibility_view_root_surfaces_state_announcements_and_platform_preferences() {
        ...
        let presentation = PearchHAPanelView.rootAccessibilityPresentation(
            snapshot: snapshot,
            preferences: PearchHAAccessibilityPreferences(reduceMotion: true, increaseContrast: true)
        )
    ...
    func test_t_accessibility_rows_expose_voiceover_labels_for_values_controls_and_failures() {
        ...
        let running = PearchHAPanelSnapshot(
            connectionState: .connected,
            phase: .connectedData,
            rooms: rooms,
            availableRooms: rooms,
        ...
        XCTAssertTrue(
            lampRow.controls.contains(
                PearchHAControlAccessibilityPresentation(
    ...
    func test_t_accessibility_keyboard_reorder_hints_follow_search_and_boundaries() {
        let unfiltered = PearchHAPanelSnapshot()
        let filtered = PearchHAPanelSnapshot(selectionQuery: "humidity")
    ...
    func testConnectionFormNormalizesFrontendURLsToUsableBaseURLs() {
        let form = PearchHAConnectionForm(
        ...
        XCTAssertEqual(
            PearchHAConnectionForm.normalizedHomeAssistantURLString(form.urlString),
            "https://ha-primary.example"
        )
        XCTAssertEqual(
            PearchHAConnectionForm.normalizedHomeAssistantURLString(form.fallbackURLString),
    ...
    func testConnectionFormKeepsProxyPrefixWhenNoFrontendRouteIsPresent() {
        let form = PearchHAConnectionForm(urlString: "https://homeassistant.local/ha")
    ...
    func testPanelConnectionFormForwardsCredentialsWithoutLeakingToken() async {
        ...
        let model = PearchHAPanelModel { form in
    ...
    func testConnectionTokenPresentationExplainsSavedKeychainToken() {
        let presentation = PearchHAConnectionTokenAccessPresentation(
    ...
    func testConnectBuildsEndpointWithAllAddressesInOrder() async throws {
        ...
        model.updateConnectionForm(
            ...
            addresses: [
                PearchHAConnectionAddressField(label: "VPN", urlString: "https://vpn.example/ha"),
                PearchHAConnectionAddressField(label: "Remote", urlString: "https://remote.example")
            ],
            token: "fake-token"
        )
        await model.connect()
    ...
    func test_t_panel_model_connects_against_fakeha() async throws {
        ...
        let model = PearchHAPanelModel(connector: fakeHAConnector)
    ...
    func test_t_history_cache_reuses_series_until_ttl_expires() async {
        ...
        let model = PearchHAPanelModel(
            ...
            historyCacheConfiguration: PearchHAHistoryCacheConfiguration(capacity: 4, ttl: .seconds(5))
        )
    ...
    func test_t_history_cache_diagnostics_report_series_and_sample_counts() async {
        ...
        let model = PearchHAPanelModel(
            ...
            historyDebounce: .milliseconds(0),
            historyCacheConfiguration: PearchHAHistoryCacheConfiguration(capacity: 4, ttl: .seconds(60))
        )
    ...
    func testHistoryCacheEvictsLeastRecentlyUsedEntryWhenCapacityIsReached() async {
        ...
        let model = PearchHAPanelModel(
            ...
            historyDebounce: .milliseconds(0),
            historyCacheConfiguration: PearchHAHistoryCacheConfiguration(capacity: 2, ttl: .seconds(60))
        )
    ...
    func test_t_history_hover_out_closes_loaded_and_unavailable_popovers() async {
        let loadedClock = TestPearchClock()
        let loadedRecorder = HistoryProviderRecorder(
            results: [
                .success(historySeries(entityID: "sensor.office_temperature", range: .hour, value: 21.4))
            ]
        )
        let loadedModel = PearchHAPanelModel(
            connector: { _ in .success(rooms: selectionRooms()) },
            historyProvider: { form, entityID, range in
        ...
        let unavailableClock = TestPearchClock()
        let unavailableModel = PearchHAPanelModel(
            connector: { _ in .success(rooms: selectionRooms()) },
    ...
    func test_t_history_empty_loaded_series_produces_empty_state() async {
        ...
        XCTAssertEqual(
            PearchHAHistoryBodyPresentation(state: model.snapshot.historyState, entityID: "sensor.office_temperature"),
            .empty
        )
    }
    ...
    func testHistoryContentSummaryNonNumericSeriesBecomesStateTimeline() {
        ...
        XCTAssertEqual(
            PearchHAHistoryContentSummary(series: series),
            .stateTimeline(
                [
                    HistoryStateSegment(
                        state: "open",
    ...
    func testHistoryBodyPresentationMapsNonNumericSeriesToStateTimeline() {
        ...
        XCTAssertEqual(
            PearchHAHistoryBodyPresentation(state: .loaded(series), entityID: "switch.office_lamp"),
    ...
    func testHistoryChartPeaksFormatMinAndMaxForCornerLabels() {
        ...
        let peaks = PearchHAHistoryPopoverContent.chartPeaks(series: series)
    ...
    func testHistoryChartPeaksHideForFlatOrNonNumericSeries() {
        ...
        XCTAssertNil(
            PearchHAHistoryPopoverContent.chartPeaks(series: flat),
        ...
        XCTAssertNil(PearchHAHistoryPopoverContent.chartPeaks(series: nonNumeric))
    }

    func testHistoryCursorReadoutIncludesDateAndTimeForDayRange() {
        let readout = PearchHAHistoryPopoverContent.cursorReadout(
            value: 53.2,
            unit: "%",
    ...
    func testHistoryStateTimelineReadoutIncludesTimeForDayRange() {
        let readout = PearchHAHistoryStateTimelinePopoverBody.readout(
            state: "bad",
            start: Date(timeIntervalSince1970: 1_200),
            duration: 7 * 60 * 60,
            range: .day,
    ...
    func testSliderValueFormattingUsesCompactPercentLabel() {
        XCTAssertEqual(PearchHASliderValueFormatting.label(for: 42, unit: "%"), "42%")
        XCTAssertEqual(PearchHASliderValueFormatting.accessibilityValue(for: 42, unit: "%"), "42 percent")
    }

    func testSliderValueFormattingUsesNumericValueAndUnitWhenPresent() {
        XCTAssertEqual(PearchHASliderValueFormatting.label(for: 21.4, unit: "°C"), "21.4 °C")
        XCTAssertEqual(PearchHASliderValueFormatting.accessibilityValue(for: 21.4, unit: "°C"), "21.4 °C")
    }
    ...
    func testHistoryContentSummaryKeepsNumericHistoryWhenTrailingSampleIsNonNumeric() {
        XCTAssertEqual(
            ...
            .statistics(PearchHAHistoryStatistics(current: 21.4, minimum: 21.4, average: 21.4, maximum: 21.4))
        )
    }
    ...
    func testHistoryContentSummaryUsesLatestChronologicalNumericSample() {
        let summary = PearchHAHistoryContentSummary(
            series: HistorySeries(
                entityID: "sensor.office_temperature",
                range: .hour,
                samples: [
        ...
        XCTAssertEqual(
            summary,
            .statistics(PearchHAHistoryStatistics(current: 30, minimum: 10, average: 20, maximum: 30))
        )
    }
    ...
    func testHistoryContentSummaryUsesTimeWeightedAverageForIrregularSamples() {
        ...
        XCTAssertEqual(
            summary,
            .statistics(PearchHAHistoryStatistics(current: 27, minimum: 26, average: 26.991803278688526, maximum: 27))
        )
    }

    func testHistoryBodyPresentationUsesSkeletonForLoadingState() {
        XCTAssertEqual(
            PearchHAHistoryBodyPresentation(
    ...
    func testHistoryBodyPresentationMapsLoadedAndUnavailableStates() {
        ...
        XCTAssertEqual(
            PearchHAHistoryBodyPresentation(state: .idle, entityID: "sensor.office_temperature"),
            .hidden
        )
    }
    ...
    func testHistorySparklineGeometryMapsTimeAndValueIntoNormalizedPoints() {
        ...
        XCTAssertEqual(
            geometry.points,
            [
                PearchHAHistorySparklinePoint(x: 0, y: 1),
                PearchHAHistorySparklinePoint(x: 0.5, y: 0),
    ...
    func testHistorySparklineGeometryFallsBackToMidlineWithoutEnoughNumericSamples() {
        let expectedMidline = [
            PearchHAHistorySparklinePoint(x: 0, y: 0.5),
            PearchHAHistorySparklinePoint(x: 1, y: 0.5)
        ]

        XCTAssertEqual(
            PearchHAHistorySparklineGeometry(
                series: HistorySeries(entityID: "sensor.office_temperature", range: .hour, samples: [])
            ).points,
            expectedMidline
        )
    ...
    func testHistorySparklineGeometryCentersConstantValuesAndSpreadsEqualTimestamps() {
        ...
        XCTAssertEqual(
            ...
            [
                PearchHAHistorySparklinePoint(x: 0, y: 0.5),
                PearchHAHistorySparklinePoint(x: 0.5, y: 0.5),
                PearchHAHistorySparklinePoint(x: 1, y: 0.5)
            ]
        )
    }
    ...
    func test_t_history_unavailable_state_is_explicit() async {
        let modelWithoutConnection = PearchHAPanelModel()
    ...
    func testBuiltInControlDerivesToggleActionsForSupportedDomains() {
        ...
        XCTAssertEqual(
            PearchHAEntityControl(entity: switchEntity)?.actionSpec(targetIsOn: true),
            ActionSpec(domain: "switch", service: "turn_on", targetEntityID: "switch.office_lamp")
        )
        XCTAssertEqual(
            PearchHAEntityControl(entity: lightEntity)?.actionSpec(targetIsOn: false),
            ActionSpec(domain: "light", service: "turn_off", targetEntityID: "light.kitchen_counter")
        )
        XCTAssertEqual(
            PearchHAEntityControl(entity: inputBoolean)?.actionSpec(targetIsOn: true),
            ActionSpec(domain: "input_boolean", service: "turn_on", targetEntityID: "input_boolean.guest_mode")
        )
        XCTAssertNil(PearchHAEntityControl(entity: sensor))
        XCTAssertNil(
            PearchHAEntityControl(
                entity: DiscoveredEntity(
    ...
    func testBuiltInCoverControlDerivesActionsAndPosition() {
        ...
        let control = PearchHACoverControl(entity: cover)
        ...
        XCTAssertNil(PearchHACoverControl(entity: sensor))
    }
    ...
    func testCustomActionRejectsIncompleteOrUnknownEntityConfiguration() async {
        let model = PearchHAPanelModel(connector: { _ in .success(rooms: selectionRooms()) })
        model.updateConnectionForm(urlString: "http://127.0.0.1:8123", token: "fake-token")
        await model.connect()
    ...
    func test_t_app_shell_custom_action_save_failure_rejects_live_configuration_change() async {
        let store = FailingConfigStore(
            saveError: .writeFailed(URL(fileURLWithPath: "/tmp/pearchha-config.json"), message: "disk full")
        )
        let action = sensorCustomAction()
        let application = PearchHAApplication(
            configStore: store,
    ...
    func testAppShellRollsBackProtectedCustomActionValueWhenSaveFails() async throws {
        let action = sensorCustomAction()
        let store = FailingConfigStore(
            loadedConfiguration: PearchHAConfiguration(customActions: [action]),
            saveError: .writeFailed(URL(fileURLWithPath: "/tmp/pearchha-config.json"), message: "disk full")
        )
        let protectedStore = InMemoryProtectedActionValueStore()
        let application = PearchHAApplication(
    ...
    func test_t_app_shell_custom_action_remove_failure_rejects_live_configuration_change() {
        ...
        let application = PearchHAApplication(configStore: store)
        application.applicationDidFinishLaunching(Notification(name: NSApplication.didFinishLaunchingNotification))
        defer {
            application.applicationWillTerminate(Notification(name: NSApplication.willTerminateNotification))
        }
    ...
    func test_t_apply_live_state_preserves_device_metadata() {
        ...
        let model = PearchHAPanelModel(
            snapshot: PearchHAPanelSnapshot(rooms: rooms, availableRooms: rooms)
        )
        model.setPanelActive(true)
    ...
    func test_t_cancelling_reconnect_does_not_mark_state_connected() async {
        ...
        let refreshGate = ConnectionGate()
        let refreshing = PearchHAPanelModel { _ in
            await refreshGate.wait()
            return .success(rooms: selectionRooms())
        }
        refreshing.updateConnectionForm(urlString: "http://127.0.0.1:8123", token: "fake-token")
        ...
        let blocked = PearchHAPanelModel { _ in
    ...
    func testPanelSnapshotRedactsTokenForDiagnostics() {
        let snapshot = PearchHAPanelSnapshot(
            connectionForm: PearchHAConnectionForm(
                urlString: "http://127.0.0.1:8123",
                fallbackURLString: "",
                token: "secret-token"
            )
    ...
    func testAppShellPanelCanBecomeKeyForFirstRunTextEntry() {
        let panel = PearchHAApplication.makePanel(model: PearchHAPanelModel())
        defer {
            panel.orderOut(nil)
    ...
    func testAppShellConnectionFormNormalizesFrontendURLsThroughNativeTextFields() throws {
        ...
        let window = PearchHAApplication.makeSettingsWindow(model: model, initialTab: .connection)
        defer {
            window.orderOut(nil)
    ...
    func testAppShellConnectionFormSupportsCommandVPasteInSettingsWindow() throws {
        ...
        // the same Edit menu `PearchHAApplication.main()` installs. Menu-claimed
        ...
        let mainMenu = PearchHAApplication.standardMainMenu()
    ...
    func testAppShellBuiltInControlsExposeNativeSwitchAndSlider() {
        ...
        let model = PearchHAPanelModel(snapshot: snapshot)
        let panel = PearchHAApplication.makePanel(model: model)
        defer {
            panel.orderOut(nil)
            panel.contentViewController = nil
        }
    ...
    func testSettingsEntitiesTabExposesNativeSearchPopupsAndKeyViewLoop() async throws {
        ...
        let window = PearchHAApplication.makeSettingsWindow(
            model: model,
            initialTab: .entities,
    ...
    func testHistoryPopoverExposesNativeRangeControlAndFocus() {
        let hostingView = NSHostingView(
            rootView: PearchHAHistoryPopoverContent(
    ...
    func test_t_app_shell_shows_loading_indicator_while_launch_autoconnect_is_in_flight() async throws {
        ...
        _ = try store.save(
            PearchHAConfiguration(
                connectionProfile: PearchHAConnectionProfile(urlString: "https://homeassistant.local:8123")
            )
        )
    ...
    func test_t_app_shell_persists_and_reloads_display_preferences() async throws {
        ...
        let preferences = PearchHADisplayPreferences(
            ...
            accentColor: PearchHAAccentColor(red: 0.1, green: 0.2, blue: 0.3),
        ...
        XCTAssertEqual(reloaded.accentColor, PearchHAAccentColor(red: 0.1, green: 0.2, blue: 0.3))
    ...
    func test_t_alert_count_ignores_hidden_modules_and_counts_only_visible_problems() async {
        ...
        let model = PearchHAPanelModel(connector: { _ in .success(rooms: rooms) })
        model.updateConnectionForm(urlString: "http://127.0.0.1:8123", token: "fake-token")
        await model.connect()
    ...
    func test_t_menu_bar_presenter_reads_promotions_from_live_snapshot() {
        let presenter = PearchHAMenuBarPresenter()
        let rooms = selectionRooms()
        let snapshot = PearchHAPanelSnapshot(
            connectionState: .connected,
            phase: .connectedData,
        ...
        let presentations = presenter.presentations(
            configuration: PearchHAConfiguration(),
    ...
    func test_t_app_shell_promotes_multiple_menu_bar_items() async throws {
        ...
        // The single-item mirror fields mirror the first promoted item.
        XCTAssertEqual(application.snapshot.statusItemTitle, "44%")
        XCTAssertEqual(application.snapshot.statusItemAccessibilityLabel, "Office humidity, 44%")
    }
    ...
    func testAppShellRefreshesStoredOAuthSessionAndRetriesConnectionOnce() async throws {
        ...
        XCTAssertEqual(try sessionStore.load().clientID, "https://pearchha.dev/app")
    }
    ...
    func testAuthorizedGatewayFallsBackAfterPrimaryTransportFailure() async throws {
        ...
        let gateway = PearchHAAuthorizedHomeAssistantGateway(client: client)
        let primaryURL = try XCTUnwrap(URL(string: "https://primary.local:8123"))
        let fallbackURL = try XCTUnwrap(URL(string: "https://fallback.example"))
        let form = PearchHAConnectionForm(
            urlString: primaryURL.absoluteString,
    ...
    func testOAuthApplicationConfigurationLoadsFromExplicitEnvironmentFile() throws {
        ...
        let configuration = try XCTUnwrap(
            try PearchHAOAuthApplicationConfiguration.fromEnvironment([
                PearchHAOAuthApplicationConfiguration.environmentFileEnvironmentKey: fileURL.path
            ])
        )

        XCTAssertEqual(configuration.clientID, "https://pearchha.dev/app")
        XCTAssertEqual(configuration.redirectURI, "pearchha://auth")
        XCTAssertEqual(configuration.callbackURLScheme, "pearchha")
    }
    ...
    func testOAuthApplicationConfigurationEnvironmentOverridesEnvironmentFile() throws {
        ...
        try """
        PEARCHHA_OAUTH_CLIENT_ID=https://pearchha.dev/file
        ...
        let configuration = try XCTUnwrap(
            try PearchHAOAuthApplicationConfiguration.fromEnvironment([
                PearchHAOAuthApplicationConfiguration.environmentFileEnvironmentKey: fileURL.path,
                PearchHAOAuthApplicationConfiguration.clientIDEnvironmentKey: "https://pearchha.dev/env",
                PearchHAOAuthApplicationConfiguration.redirectURIEnvironmentKey: "pearchha-env://auth"
            ])
        )

        XCTAssertEqual(configuration.clientID, "https://pearchha.dev/env")
        XCTAssertEqual(configuration.redirectURI, "pearchha-env://auth")
        XCTAssertEqual(configuration.callbackURLScheme, "pearchha-env")
    }
    ...
    func testOAuthApplicationConfigurationEnvironmentOverridesMissingEnvironmentFile() throws {
        let missingFileURL = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("pearchha-missing-env-\(UUID().uuidString)", isDirectory: false)

        let configuration = try XCTUnwrap(
            try PearchHAOAuthApplicationConfiguration.fromEnvironment([
                PearchHAOAuthApplicationConfiguration.environmentFileEnvironmentKey: missingFileURL.path,
                PearchHAOAuthApplicationConfiguration.clientIDEnvironmentKey: "https://pearchha.dev/env",
                PearchHAOAuthApplicationConfiguration.redirectURIEnvironmentKey: "pearchha-env://auth"
            ])
        )
    ...
    func testOAuthApplicationConfigurationBlankEnvironmentValueOverridesEnvironmentFile() throws {
        ...
        XCTAssertThrowsError(
            try PearchHAOAuthApplicationConfiguration.fromEnvironment([
                PearchHAOAuthApplicationConfiguration.environmentFileEnvironmentKey: fileURL.path,
                PearchHAOAuthApplicationConfiguration.clientIDEnvironmentKey: " ",
                PearchHAOAuthApplicationConfiguration.redirectURIEnvironmentKey: "pearchha-env://auth"
            ])
        ) { error in
            XCTAssertEqual(
                error as? PearchHAOAuthSignInFailure,
    ...
    func testOAuthApplicationConfigurationRejectsMalformedEnvironmentFileLine() {
        XCTAssertThrowsError(
            try PearchHAOAuthApplicationConfiguration.parseEnvironmentFile(
                """
                PEARCHHA_OAUTH_CLIENT_ID
                PEARCHHA_OAUTH_REDIRECT_URI=pearchha://auth
                """
            ...
            XCTAssertEqual(error as? PearchHAOAuthEnvironmentFileError, .invalidLine(1))
        }
    }
    ...
    func testAppShellApplicationOpenAcceptsConfiguredOAuthCallbackWithoutRetainingSecrets() throws {
        let application = PearchHAApplication(
            ...
            oauthApplicationConfiguration: try PearchHAOAuthApplicationConfiguration(
                clientID: "https://pearchha.dev/app",
                redirectURI: "pearchha://auth"
            )
        )
        let callbackURL = try XCTUnwrap(URL(string: "pearchha://auth?code=secret-code&state=secret-state"))
        ...
        XCTAssertEqual(event.scheme, "pearchha")
    ...
    func testAppShellExternalOAuthCallbackRejectsMismatchedRedirectBase() throws {
        ...
        let callbackURL = try XCTUnwrap(URL(string: "pearchha://other?code=secret-code&state=secret-state"))

        let events = application.handleExternalURLs([callbackURL])

        XCTAssertEqual(events.first?.scheme, "pearchha")
    ...
    func testOAuthSignInCoordinatorStoresSessionFromCallback() async throws {
        ...
        let presenter = OAuthPresenterRecorder(
            result: .callback(try XCTUnwrap(URL(string: "pearchha://auth?code=auth-code&state=state-value")))
        )
        let coordinator = PearchHAOAuthSignInCoordinator(
            configuration: try PearchHAOAuthApplicationConfiguration(
                clientID: "https://pearchha.dev/app",
                redirectURI: "pearchha://auth"
            ),
        ...
        XCTAssertEqual(
            _mlHoisted1012,
            try XCTUnwrap(URL(string: "https://homeassistant.local:8123/auth/authorize?client_id=https%3A%2F%2Fpearchha.dev%2Fapp&redirect_uri=pearchha%3A%2F%2Fauth&state=state-value"))
        )
        ...
        XCTAssertEqual(
            ...
            "grant_type=authorization_code&code=auth-code&client_id=https%3A%2F%2Fpearchha.dev%2Fapp"
        )
    ...
    func testOAuthSignInCoordinatorRejectsMismatchedStateWithoutStoringSession() async throws {
        ...
        let presenter = OAuthPresenterRecorder(
            result: .callback(try XCTUnwrap(URL(string: "pearchha://auth?code=auth-code&state=wrong-state")))
        )
        let coordinator = PearchHAOAuthSignInCoordinator(
            configuration: try PearchHAOAuthApplicationConfiguration(
                clientID: "https://pearchha.dev/app",
        ...
        let result = await coordinator.signIn(
            form: PearchHAConnectionForm(urlString: "http://homeassistant.local:8123")
        )
    ...
    func testOAuthSignInCoordinatorRejectsMismatchedRedirectURIWithoutStoringSession() async throws {
        ...
        let transport = OAuthHARESTTransportRecorder(responses: [])
        let presenter = OAuthPresenterRecorder(
            result: .callback(try XCTUnwrap(URL(string: "pearchha://other?code=auth-code&state=state-value")))
        )
        let coordinator = PearchHAOAuthSignInCoordinator(
            configuration: try PearchHAOAuthApplicationConfiguration(
                clientID: "https://pearchha.dev/app",
    ...
    func testPanelOAuthSignInConnectsWithAuthorizedFormWhenFieldsChangeDuringSignIn() async {
        let recorder = ConnectionFormRecorder()
        var model: PearchHAPanelModel!
        model = PearchHAPanelModel(
            connector: { form in
                await recorder.record(form)
                return .success(rooms: [])
            },
    ...
    func testPanelCheckForUpdatesPublishesAvailableRelease() async throws {
        let releaseURL = try XCTUnwrap(URL(string: "https://github.com/YunaBraska/pearch_ha/releases/tag/2026.7.7"))
        let downloadURL = try XCTUnwrap(URL(string: "https://github.com/YunaBraska/pearch_ha/releases/download/2026.7.7/PearchHA-2026.7.7.dmg"))
        let model = PearchHAPanelModel(
            releaseUpdateChecker: { currentVersion in
                XCTAssertEqual(currentVersion, "1.0")
                return .updateAvailable(
                    PearchHAReleaseUpdate(
                        currentVersion: "1.0",
                        latestVersion: "2026.7.7",
    ...
    func testGitHubReleaseUpdateCheckerPrefersDMGAssetAndReportsAvailableRelease() async throws {
        let checker = PearchHAGitHubReleaseUpdateChecker { request in
            XCTAssertEqual(request.value(forHTTPHeaderField: "Accept"), "application/vnd.github+json")
            XCTAssertEqual(request.value(forHTTPHeaderField: "X-GitHub-Api-Version"), "2026-03-10")
            XCTAssertEqual(request.value(forHTTPHeaderField: "User-Agent"), "PearchHA/1.0")
            let payload = """
            {
              "tag_name": "2026.7.7",
              "html_url": "https://github.com/YunaBraska/pearch_ha/releases/tag/2026.7.7",
              "assets": [
                {
                  "name": "PearchHA-2026.7.7.zip",
                  "browser_download_url": "https://github.com/YunaBraska/pearch_ha/releases/download/2026.7.7/PearchHA-2026.7.7.zip"
                },
                {
                  "name": "PearchHA-2026.7.7.dmg",
                  "browser_download_url": "https://github.com/YunaBraska/pearch_ha/releases/download/2026.7.7/PearchHA-2026.7.7.dmg"
                }
              ]
            }
            """
        ...
        XCTAssertEqual(
            ...
            .updateAvailable(
                PearchHAReleaseUpdate(
                    currentVersion: "1.0",
                    latestVersion: "2026.7.7",
                    releaseURL: try XCTUnwrap(URL(string: "https://github.com/YunaBraska/pearch_ha/releases/tag/2026.7.7")),
                    downloadURL: try XCTUnwrap(URL(string: "https://github.com/YunaBraska/pearch_ha/releases/download/2026.7.7/PearchHA-2026.7.7.dmg"))
                )
            )
        )
    }
    ...
    func testGitHubReleaseUpdateCheckerReturnsUpToDateWhenLatestReleaseIsInstalled() async throws {
        ...
        let result = await checker.checkLatest(currentVersion: "2026.7.7")

        XCTAssertEqual(
            result,
            .upToDate(
                currentVersion: "2026.7.7",
                latestVersion: "2026.7.7",
                releaseURL: try XCTUnwrap(URL(string: "https://github.com/YunaBraska/pearch_ha/releases/tag/2026.7.7"))
            )
        )
        ...
        let checker = PearchHAGitHubReleaseUpdateChecker { request in
            let payload = """
            {
              ...
              "assets": [
                {
                  "name": "checksums.txt",
                  "browser_download_url": "https://github.com/YunaBraska/pearch_ha/releases/download/2026.7.7/checksums.txt"
                }
              ]
            }
            """
        ...
        XCTAssertEqual(
            ...
            .updateAvailable(
                PearchHAReleaseUpdate(
                    ...
                    latestVersion: "2026.7.7",
                    releaseURL: try XCTUnwrap(URL(string: "https://github.com/YunaBraska/pearch_ha/releases/tag/2026.7.7")),
                    downloadURL: try XCTUnwrap(URL(string: "https://github.com/YunaBraska/pearch_ha/releases/tag/2026.7.7"))
                )
            )
        )
    }

    func testGitHubReleaseUpdateCheckerPrefersPearchHAArtifactWhenOtherDMGsExist() async throws {
        let checker = PearchHAGitHubReleaseUpdateChecker { request in
            let payload = """
            {
              "tag_name": "2026.7.7",
              ...
              "assets": [
                {
                  "name": "OtherTool.dmg",
                  "browser_download_url": "https://github.com/YunaBraska/pearch_ha/releases/download/2026.7.7/OtherTool.dmg"
                },
                {
                  "name": "PearchHA-2026.7.7.zip",
                  "browser_download_url": "https://github.com/YunaBraska/pearch_ha/releases/download/2026.7.7/PearchHA-2026.7.7.zip"
        ...
        XCTAssertEqual(
            ...
            .updateAvailable(
                PearchHAReleaseUpdate(
                    ...
                    latestVersion: "2026.7.7",
                    releaseURL: try XCTUnwrap(URL(string: "https://github.com/YunaBraska/pearch_ha/releases/tag/2026.7.7")),
                    downloadURL: try XCTUnwrap(URL(string: "https://github.com/YunaBraska/pearch_ha/releases/download/2026.7.7/PearchHA-2026.7.7.zip"))
                )
            )
        )
    }
    ...
    func testGitHubReleaseUpdateCheckerAcceptsVersionPrefixAndReportsUpToDate() async throws {
        let checker = PearchHAGitHubReleaseUpdateChecker { request in
            ...
            {
              "tag_name": "v2026.7.7",
              "html_url": "https://github.com/YunaBraska/pearch_ha/releases/tag/v2026.7.7",
              "assets": []
            }
            """
            return (
        ...
        XCTAssertEqual(
            ...
            .upToDate(
                currentVersion: "2026.7.7",
                latestVersion: "v2026.7.7",
                releaseURL: try XCTUnwrap(URL(string: "https://github.com/YunaBraska/pearch_ha/releases/tag/v2026.7.7"))
            )
        )
    }
    ...
    func testGitHubReleaseUpdateCheckerRejectsInvalidCurrentVersionExplicitly() async throws {
        let checker = PearchHAGitHubReleaseUpdateChecker { _ in
    ...
    func test_t_status_item_gauge_image_renderer_draws_severity_colors() {
        let palette = PearchHAStatusItemGaugePalette(
        ...
        let renderer = PearchHAStatusItemGaugeImageRenderer(
    ...
    func test_t_display_total_threshold_settings_reject_invalid_inputs() async throws {
        ...
        XCTAssertEqual(
            ...
            "the warning setter maps to an orange step"
        )
    }

    func test_t_display_settings_save_failure_rejects_live_status_item_change() async {
        let store = FailingConfigStore(
            loadedConfiguration: PearchHAConfiguration(
                selectedEntityIDs: ["sensor.office_humidity"],
                menuBarEntityIDs: ["sensor.office_humidity"],
                isEntitySelectionExplicit: true
            ),
    ...
    func test_t_app_shell_reorder_save_survives_relaunch() async throws {
        ...
        let firstApplication = PearchHAApplication(configStore: store)
        ...
        let secondApplication = PearchHAApplication(configStore: store)
    ...
    func test_t_average_links_persist_and_survive_application_relaunch() async throws {
        ...
        let secondApplication = PearchHAApplication(
            configStore: store,
            connector: { _ in .success(rooms: averageRooms()) }
        )
        secondApplication.applicationDidFinishLaunching(Notification(name: NSApplication.didFinishLaunchingNotification))
    ...
    func testAppShellRemembersConnectionProfileAndStoredAccessTokenAcrossRelaunch() async throws {
        ...
        let firstApplication = PearchHAApplication(
    ...
    func test_t_keychain_save_failure_when_remembering_session_surfaces_in_settings() async throws {
        ...
        let application = PearchHAApplication(
            configStore: JSONConfigStore(fileURL: url),
            authSessionStore: PearchHAAuthSessionStore(secretStore: secretStore),
            client: RefreshingHAClientRecorder(
                discoveryResults: [.success(oauthDiscoverySnapshot())]
            )
        )
    ...
    func testAppShellAutoConnectsOnLaunchWhenStoredSessionAndProfileExist() async throws {
        ...
        let seedApplication = PearchHAApplication(
            configStore: JSONConfigStore(fileURL: url),
            authSessionStore: sessionStore,
            client: RefreshingHAClientRecorder(
                discoveryResults: [.success(oauthDiscoverySnapshot())]
        ...
        let relaunched = PearchHAApplication(
            configStore: JSONConfigStore(fileURL: url),
            authSessionStore: sessionStore,
            client: RefreshingHAClientRecorder(
                discoveryResults: [.success(oauthDiscoverySnapshot())]
    ...
    func testAppShellAutoConnectRecoversWhenInitialStoredTokenProbeFailsOnce() async throws {
        ...
        let sessionStore = PearchHAAuthSessionStore(secretStore: secretStore)
        _ = try secretStore.save("long-lived-token", for: .accessToken)
        _ = try JSONConfigStore(fileURL: url).save(
            PearchHAConfiguration(
                connectionProfile: PearchHAConnectionProfile(
                    addresses: [PearchHAConnectionAddress(urlString: "https://homeassistant.local:8123")]
                )
            )
        )
    ...
    func testAppShellAutoConnectContinuesWhenDeviceRegistryPayloadIsMissing() async throws {
        ...
        _ = try JSONConfigStore(fileURL: url).save(
            PearchHAConfiguration(
                connectionProfile: PearchHAConnectionProfile(
                    addresses: [PearchHAConnectionAddress(urlString: server.baseURL.absoluteString)]
                )
            )
        )
    ...
    func test_t_app_shell_reports_config_load_failure_and_blocks_save() {
        let store = FailingConfigStore(
            loadError: .malformedConfig(URL(fileURLWithPath: "/tmp/pearchha-bad-config.json"), message: "bad json")
        )
        let application = PearchHAApplication(configStore: store)
        application.applicationDidFinishLaunching(Notification(name: NSApplication.didFinishLaunchingNotification))
        defer {
    ...
    private func makeConnectedBulkSyncModel(
        ...
        clock: TestPearchClock,
        rooms: [Room],
        bulkSync: PearchHAHistoryBulkSyncConfiguration,
        historyProvider: @Sendable @escaping (PearchHAConnectionForm, EntityID, HistoryRange) async -> PearchHAHistoryProviderResult = {
        ...
        let model = PearchHAPanelModel(
            ...
            historyCacheConfiguration: PearchHAHistoryCacheConfiguration(capacity: capacity, ttl: cacheTTL),
    ...
    func test_t_bulk_sync_syncs_all_displayed_entities_in_one_request() async {
        ...
        let settleDelay = PearchDuration.milliseconds(250)
    ...
    func test_t_visible_history_background_sync_counts_each_batch_in_requests_per_minute() async {
        ...
        let model = PearchHAPanelModel(
            ...
            clock: clock,
            wallClock: { dates.now },
            bulkSyncConfiguration: PearchHAHistoryBulkSyncConfiguration(settleDelay: settleDelay, batchSize: 40, coldRefreshDivisor: 1),
            periodicRefreshConfiguration: .disabled
        )
        model.updateConnectionForm(urlString: "http://127.0.0.1:8123", token: "fake-token")
        await model.connect()
    ...
    func test_t_visible_row_warms_day_week_and_month_layers_when_long_ranges_are_cold() async {
        ...
        let model = await makeConnectedBulkSyncModel(
            ...
            rooms: prefetchRooms(count: 1),
            bulkSync: PearchHAHistoryBulkSyncConfiguration(settleDelay: settleDelay, batchSize: 40, coldRefreshDivisor: 1),
            historyDebounce: PearchDuration.seconds(600)
        )
        XCTAssertTrue(model.setMenuBarDefaultHistoryRange("sensor.prefetch_0", defaultHistoryRange: HistoryRange.hour))
    ...
    func test_t_opened_history_detail_reuses_warmed_month_cache_without_refetching() async {
        ...
        _ = await clock.advance(by: PearchDuration.seconds(120))
    ...
    func test_t_opened_history_detail_refreshes_week_and_month_only_after_24_hours() async {
        ...
        let recorder = BulkHistoryRecorder()
        let settleDelay = PearchDuration.milliseconds(250)
        let interval = PearchDuration.seconds(30)
        let model = await makeConnectedBulkSyncModel(
            recorder: recorder,
            clock: clock,
            rooms: prefetchRooms(count: 1),
            bulkSync: PearchHAHistoryBulkSyncConfiguration(interval: interval, settleDelay: settleDelay, batchSize: 40, coldRefreshDivisor: 1),
            historyDebounce: PearchDuration.seconds(600)
        )
        XCTAssertTrue(model.setMenuBarDefaultHistoryRange("sensor.prefetch_0", defaultHistoryRange: HistoryRange.hour))
    ...
    func test_t_scroll_pause_rearm_does_not_refetch_recently_synced_entities() async {
        ...
        XCTAssertEqual(afterFirstCycle - baselineBatches, 3)
        ...
        // Narrowing the visible set rearms the loop, but recently synced layers
        // stay warm. Depending on scheduling, the re-arm may do nothing or may
        // emit one immediate maintenance batch for the still-visible row; it must
        // never refetch the removed row or fan out into multiple fresh batches.
        model.updateVisibleEntities(["sensor.prefetch_0"])
        await spinUntil { await clock.sleepingTaskCount() == 1 }
        _ = await clock.advance(by: settleDelay)
        await spinUntil { await clock.sleepingTaskCount() == 1 }
        ...
        XCTAssertLessThanOrEqual(
            afterRearm - afterFirstCycle,
            1,
            "a re-arm within the interval should add at most one maintenance batch"
        )
        let removedRowRefetches = await recorder.requestCountForEntity(
            EntityID("sensor.prefetch_1"),
            afterBatchCount: afterFirstCycle
        )
        XCTAssertEqual(
            removedRowRefetches,
            0,
            "a removed row must not be refetched during the re-arm"
        )

        // After a full interval the same visible entity is due again.
        await spinUntil { await clock.sleepingTaskCount() == 1 }
        _ = await clock.advance(by: interval)
        await spinUntil { await recorder.batchCount() >= afterRearm + 1 }
        model.setPanelActive(false)
    }
    ...
    func test_t_bulk_history_refreshes_expired_oauth_session_and_retries_once() async throws {
        ...
        let gateway = PearchHAAuthorizedHomeAssistantGateway(client: client, authSessionStore: sessionStore)
        let form = PearchHAConnectionForm(urlString: "http://homeassistant.local:8123", usesStoredAuthSession: true)
    ...
    func test_t_bulk_sync_pauses_while_connection_failed_and_resumes_on_recovery() async {
        ...
        let model = PearchHAPanelModel(
            ...
            clock: clock,
            bulkSyncConfiguration: PearchHAHistoryBulkSyncConfiguration(interval: interval, settleDelay: settleDelay, coldRefreshDivisor: 1),
            periodicRefreshConfiguration: .disabled
        )
        model.updateConnectionForm(urlString: "http://127.0.0.1:8123", token: "fake-token")
        await model.connect()
    ...
    func test_t_inline_history_survives_ttl_expiry_for_display() async {
        ...
        let model = await makeConnectedBulkSyncModel(
            ...
            bulkSync: PearchHAHistoryBulkSyncConfiguration(interval: .seconds(120), settleDelay: settleDelay, coldRefreshDivisor: 1),
            cacheTTL: .seconds(60)
        )
    ...
    func test_t_gauge_meter_shows_for_available_and_stale_not_for_missing() {
        XCTAssertTrue(PearchHADashboardPreview.meterShows(for: .available))
        // Last-known fraction stays on screen through a background sync / WS gap.
        XCTAssertTrue(PearchHADashboardPreview.meterShows(for: .stale))
        XCTAssertFalse(PearchHADashboardPreview.meterShows(for: .unavailable))
        XCTAssertFalse(PearchHADashboardPreview.meterShows(for: .unknown))
    }
    ...
    func test_t_background_periodic_refresh_keeps_session_connected_and_not_stale() async {
        ...
        let calls = CallCounter()
        let clock = TestPearchClock()
        var model: PearchHAPanelModel? = PearchHAPanelModel(
            connector: { _ in
    ...
    func test_t_same_connection_ignores_address_row_identity() {
        let a = PearchHAConnectionForm(
            urlString: "http://home:8123",
            addresses: [PearchHAConnectionAddressField(label: "VPN", urlString: "http://vpn:8123")],
            token: "tok"
        )
        let b = PearchHAConnectionForm(
            urlString: "http://home:8123",
            addresses: [PearchHAConnectionAddressField(label: "VPN", urlString: "http://vpn:8123")],
            token: "tok"
        )
        ...
        XCTAssertFalse(a.sameConnection(as: PearchHAConnectionForm(urlString: "http://other:8123", token: "tok")))
        XCTAssertFalse(a.sameConnection(as: PearchHAConnectionForm(
            urlString: "http://home:8123",
            addresses: [PearchHAConnectionAddressField(urlString: "http://vpn:8123")],
            token: "different"
        )))
    }

    func test_t_same_connection_differs_when_self_signed_opt_in_changes() {
        let trusting = PearchHAConnectionForm(urlString: "https://home:8123", token: "tok")
        var strict = trusting
    ...
    func test_t_self_signed_hosts_are_scoped_to_https_addresses_only() {
        let form = PearchHAConnectionForm(
            ...
            addresses: [
                PearchHAConnectionAddressField(label: "VPN", urlString: "https://vpn.example/ha"),
                PearchHAConnectionAddressField(label: "LAN", urlString: "http://plain.local:8123"),
                PearchHAConnectionAddressField(label: "Broken", urlString: "not a url")
            ],
            token: "tok",
            allowsSelfSignedCertificates: true
        )
        ...
        let httpOnly = PearchHAConnectionForm(urlString: "http://plain.local:8123", token: "tok")
    ...
    func test_t_reapplying_equivalent_form_keeps_history_cache() async {
        ...
        model.updateConnectionForm(
            urlString: "http://127.0.0.1:8123",
            addresses: [PearchHAConnectionAddressField(urlString: "http://10.0.0.2:8123")],
            token: "fake-token"
        )
        await model.connect()
        await model.loadHistory("sensor.office_temperature")
    ...
    func test_t_inactive_panel_primes_history_once_then_stays_push_driven() async {
        ...
        let model = PearchHAPanelModel(
            ...
            clock: clock,
            bulkSyncConfiguration: PearchHAHistoryBulkSyncConfiguration(settleDelay: .milliseconds(250)),
            periodicRefreshConfiguration: .disabled,
    ...
    func test_t_inline_sparkline_geometry_downsamples_to_budget() {
        ...
        let full = PearchHAHistorySparklineGeometry(series: series)
        let capped = PearchHAHistorySparklineGeometry(series: series, maxSamples: 48)
    ...
    func test_t_model_with_active_background_loops_deallocates_when_released() async {
        let clock = TestPearchClock()
        weak var weakModel: PearchHAPanelModel?
        do {
            let model = PearchHAPanelModel(
                connector: { _ in .success(rooms: prefetchRooms(count: 1)) },
                clock: clock,
                bulkSyncConfiguration: PearchHAHistoryBulkSyncConfiguration(),
                periodicRefreshConfiguration: PearchHAPeriodicRefreshConfiguration()
            )
            model.updateConnectionForm(urlString: "http://127.0.0.1:8123", token: "fake-token")
            await model.connect()
            model.setPanelActive(true)
    ...
    func test_t_apply_live_state_keeps_descriptive_name_and_unit_when_live_update_is_generic() {
        ...
        let model = PearchHAPanelModel(
            snapshot: PearchHAPanelSnapshot(
                connectionState: .connected,
                phase: .connectedData,
                rooms: rooms,
                availableRooms: rooms
    ...
    private func temporaryConfigURL() -> URL {
        URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("pearchha-ui-test-\(UUID().uuidString)", isDirectory: true)
    ...
    func testDashboardPaletteResolvesDistinctDesignedLightAndDarkSurfaces() {
        let dark = PearchHATheme.Dashboard.palette(.dark)
        let light = PearchHATheme.Dashboard.palette(.light)
        XCTAssertEqual(dark, PearchHATheme.Dashboard.darkPalette)
        XCTAssertEqual(light, PearchHATheme.Dashboard.lightPalette)
    ...
    func testDashboardPaletteCompatibilityAliasesTrackTheNewTokens() {
        let palette = PearchHATheme.Dashboard.darkPalette
    ...
    private func connectedDashboardSnapshot() -> PearchHAPanelSnapshot {
        let rooms = selectionRooms()
        return PearchHAPanelSnapshot(
            connectionState: .connected,
            phase: .connectedData,
            rooms: rooms,
            availableRooms: rooms
    ...
    func test_t_diagnostics_ring_buffer_evicts_oldest_beyond_capacity() async {
        ...
        XCTAssertEqual(model.diagnosticEvents.count, PearchHADiagnosticLog.defaultCapacity)
    ...
    func test_t_diagnostics_retry_backoff_state_reflects_periodic_backoff() async {
        ...
        let model = PearchHAPanelModel(
            connector: { _ in await outcomes.next() },
            clock: clock,
            periodicRefreshConfiguration: PearchHAPeriodicRefreshConfiguration(interval: interval, maximumBackoff: .seconds(300))
        )
        model.updateConnectionForm(urlString: "http://127.0.0.1:8123", token: "fake-token")
        await model.connect()
...
@MainActor
private final class OAuthPresenterRecorder: PearchHAOAuthAuthorizationPresenter, @unchecked Sendable {
    private let result: PearchHAOAuthPresentationResult
    private var recordedAuthorizationURLs: [URL] = []

    init(result: PearchHAOAuthPresentationResult) {
        self.result = result
    }
    ...
    ) async -> PearchHAOAuthPresentationResult {
...
private final class ScriptableFailureSecretStore: SecretStore, @unchecked Sendable {
    ...
    private var values: [PearchHASecret: String] = [:]

    @discardableResult
    func save(_ value: String, for secret: PearchHASecret) throws -> SecretWriteResult {
        if failsSave {
    ...
    @discardableResult
    func delete(_ secret: PearchHASecret) throws -> SecretDeleteResult {
        if failsDelete {
...
private actor RefreshingHAClientRecorder: PearchHAServerTrustRefreshingHomeAssistantClient {
...
private actor ConnectionFormRecorder {
    ...
    func record(_ form: PearchHAConnectionForm) {
        recordedCallCount += 1
...
private actor ConnectionResultRecorder {
    private var results: [PearchHAConnectionAttemptResult]
    private var recordedCallCount = 0

    init(results: [PearchHAConnectionAttemptResult]) {
        self.results = results
    }

    func next() -> PearchHAConnectionAttemptResult {
        recordedCallCount += 1
        if results.isEmpty {
...
private actor HistoryProviderRecorder {
    private var results: [PearchHAHistoryProviderResult]
    ...
    func provide(
        ...
        entityID: EntityID,
        range: HistoryRange
    ) async -> PearchHAHistoryProviderResult {
...
private actor ActionRunnerRecorder {
    private var results: [PearchHAActionResult]
    private let waitForRelease: Bool
    private var recordedForms: [PearchHAConnectionForm] = []
    ...
    func run(form: PearchHAConnectionForm, action: ActionSpec) async -> PearchHAActionResult {
        recordedForms.append(form)
...
@MainActor
private final class CountingStatusItemGaugeImageRenderer: PearchHAStatusItemGaugeImageRendering {
    private let renderer = PearchHAStatusItemGaugeImageRenderer()
...
private extension PearchHAPanelModel {
...
private extension PearchHAApplication {
...
private final class FailingConfigStore: ConfigStore, @unchecked Sendable {
    private let loadedConfiguration: PearchHAConfiguration
    private let loadError: ConfigStoreError?
    private let saveError: ConfigStoreError?
    private(set) var saveCallCount = 0

    init(
        loadedConfiguration: PearchHAConfiguration = .empty,
    ...
    @discardableResult
    func save(_ configuration: PearchHAConfiguration) throws -> PearchHAConfiguration {
        saveCallCount += 1
        if let saveError {
            throw saveError
        }
...
func fakeHAConnector(form: PearchHAConnectionForm) async -> PearchHAConnectionAttemptResult {
    guard let primaryURL = form.primaryURL() else {
        return .failure(.protocolError("invalid Home Assistant URL"))
    }
    let input = HAConnectionInput(
...
private actor PeriodicConnectorOutcomes {
    private var results: [PearchHAConnectionAttemptResult]
    private let fallback: PearchHAConnectionAttemptResult

    init(results: [PearchHAConnectionAttemptResult], fallback: PearchHAConnectionAttemptResult) {
        self.results = results
...
extension PearchHAUITests {
    ...
    func test_t_pearch_application_icon_resource_loads() throws {
        ...
        let icon = try XCTUnwrap(PearchHAApplication.pearchApplicationIcon())
