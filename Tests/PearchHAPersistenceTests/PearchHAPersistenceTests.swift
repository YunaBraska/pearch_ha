...
final class PearchHAPersistenceTests: XCTestCase {
    func testConfigLocationDefaults() {
        let location = ConfigLocation()

        XCTAssertEqual(location.applicationSupportDirectoryName, "PearchHA")
    ...
    func testConfigurationDefaultsDisplayPreferences() {
        let configuration = PearchHAConfiguration.empty
    ...
    func testJSONConfigStoreRoundTripsDisplayPreferences() throws {
        ...
        let configuration = PearchHAConfiguration(
            ...
            accentColor: PearchHAAccentColor(red: 0.2, green: 0.4, blue: 0.6, alpha: 0.9),
    ...
    func testConfigurationApplyingDisplayPreferencesReplacesOnlyDisplayFields() {
        let base = PearchHAConfiguration(
            selectedEntityIDs: ["sensor.office_temperature"],
            menuBarEntityIDs: ["sensor.office_temperature"]
        )
        let preferences = PearchHADisplayPreferences.defaults
    ...
    func testConfigurationDefaultsSummaryMetricEntityIDsToEmpty() {
        XCTAssertTrue(PearchHAConfiguration.empty.dashboardSummaryMetricEntityIDs.isEmpty)
        XCTAssertTrue(PearchHAConfiguration.empty.displayPreferences.summaryMetricEntityIDs.isEmpty)
    }
    ...
    func testApplyingDisplayPreferencesCarriesSummaryMetricEntityIDs() {
        let base = PearchHAConfiguration(selectedEntityIDs: ["sensor.office_temperature"])
        let preferences = PearchHADisplayPreferences.defaults
    ...
    func testDisplayPreferencesCapsAndDeduplicatesSummaryMetricEntityIDs() {
        let prefs = PearchHADisplayPreferences.defaults
    ...
    func testAccentColorClampsChannelsIntoUnitRange() {
        let accent = PearchHAAccentColor(red: -0.5, green: 2, blue: 0.3, alpha: 5)
    ...
    func testAccentColorMatchesNamedSwatch() {
        XCTAssertEqual(PearchHAAccentColor.homeAssistantBlue.matchingSwatchID, "ha-blue")
        XCTAssertNil(PearchHAAccentColor(red: 0.123, green: 0.456, blue: 0.789).matchingSwatchID)
    }

    func testDisplayPreferencesWithReplacesSingleField() {
        let base = PearchHADisplayPreferences.defaults
    ...
    func testJSONConfigStoreRejectsUnsupportedSchemaWithoutOverwritingExistingConfig() throws {
        ...
        let store = JSONConfigStore(fileURL: url)
        let valid = PearchHAConfiguration(selectedEntityIDs: ["sensor.office_temperature"])
        let unsupported = PearchHAConfiguration(schemaVersion: 999, selectedEntityIDs: ["sensor.bad"])
    ...
    func testConfigStoreWritesAtomicallyEnoughToLeaveOneReadableJSONFile() throws {
        ...
        let store = JSONConfigStore(fileURL: url)
        let first = PearchHAConfiguration(selectedEntityIDs: ["sensor.first"])
        let second = PearchHAConfiguration(selectedEntityIDs: ["sensor.second"], roomOrder: ["lab"])
        ...
        XCTAssertNoThrow(try JSONDecoder().decode(PearchHAConfiguration.self, from: data))
    ...
    func test_t_secret_storage_and_redaction() throws {
        let store = KeychainSecretStore(service: "dev.pearchha.tests.\(UUID().uuidString)")
        defer {
    ...
    func testKeychainProtectedActionValueStoreRoundsTripsAndDeletesValues() throws {
        let secretStore = KeychainSecretStore(service: "dev.pearchha.tests.\(UUID().uuidString)")
    ...
    func testAuthSessionStoreSavesRotatesLoadsAndClearsTokens() throws {
        let keychain = KeychainSecretStore(service: "dev.pearchha.tests.\(UUID().uuidString)")
        let store = PearchHAAuthSessionStore(secretStore: keychain)
        defer {
            _ = try? store.clear()
        }

        let session = try PearchHAAuthSession(
        ...
        XCTAssertEqual(
            ...
            PearchHAAuthSessionWriteResult(accessToken: .created, refreshToken: .created)
        )
        XCTAssertEqual(
            try store.load(),
            try PearchHAAuthSession(
                accessToken: "access-token",
                refreshToken: "refresh-token",
                clientID: "https://pearchha.dev/app"
            )
        ...
        XCTAssertEqual(
            try store.clear(),
            PearchHAAuthSessionClearResult(accessToken: .deleted, refreshToken: .deleted)
        )
    ...
    func testAuthSessionRejectsEmptyTokensBeforeWritingToKeychain() throws {
        XCTAssertThrowsError(try PearchHAAuthSession(accessToken: "", refreshToken: "refresh-token")) { error in
            XCTAssertEqual(error as? SecretStoreError, .emptySecret(.accessToken))
        }
        XCTAssertThrowsError(try PearchHAAuthSession(accessToken: "access-token", refreshToken: " ")) { error in
            XCTAssertEqual(error as? SecretStoreError, .emptySecret(.refreshToken))
        }
        XCTAssertThrowsError(try PearchHAAuthSession(accessToken: "access-token", refreshToken: "refresh-token", clientID: " ")) { error in
    ...
    func testConnectionProfileWithMultipleAddressesRoundTrips() throws {
        ...
        let configuration = PearchHAConfiguration(
            connectionProfile: PearchHAConnectionProfile(addresses: [
                PearchHAConnectionAddress(label: "Home", urlString: "https://home.local:8123"),
                PearchHAConnectionAddress(label: "VPN", urlString: "https://vpn.example/ha"),
                PearchHAConnectionAddress(label: "Remote", urlString: "https://remote.example")
            ])
        )
        let store = JSONConfigStore(fileURL: url)
    ...
    func testConnectionProfileRoundTripsStrictCertificateValidationChoice() throws {
        ...
        let configuration = PearchHAConfiguration(
            connectionProfile: PearchHAConnectionProfile(
                addresses: [PearchHAConnectionAddress(urlString: "https://home.local:8123")],
    ...
    private func temporaryConfigURL() -> URL {
        URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("pearchha-persistence-test-\(UUID().uuidString)", isDirectory: true)
            .appendingPathComponent("config.json", isDirectory: false)
    }
}

final class FailureInjectingSecretStore: SecretStore, @unchecked Sendable {
    var saveFailures: [PearchHASecret: SecretStoreError] = [:]
    var deleteFailures: [PearchHASecret: SecretStoreError] = [:]
