...
final class PearchHARepoAuditTests: XCTestCase {
    ...
    func testRepositoryAuditRejectsTelemetryReferencesInXcodeProject() throws {
        ...
        let dependencyURL = "https://github.com/" + "get" + "sentry" + "/" + "sentry" + "-cocoa"
        try writeFile(
            "PearchHA.xcodeproj/project.pbxproj",
            contents: "repositoryURL = \(dependencyURL);\n",
            in: repository
        )
        try git(["add", "PearchHA.xcodeproj/project.pbxproj"], in: repository)

        let result = runRepoAudit(rootURL: repository)

        XCTAssertEqual(result.exitCode, 1)
        XCTAssertTrue(result.errorText.contains("PearchHA.xcodeproj/project.pbxproj: telemetry dependency or collection endpoint must not be present by default"))
    }
    ...
    private func temporaryDirectory() -> URL {
        URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("PearchHARepoAuditTests-\(UUID().uuidString)", isDirectory: true)
    }
