...
- `.github/workflows/release.yml` provides a branch-safe `local-evidence` workflow-dispatch path plus a credentialed Developer ID + notarization workflow-dispatch path that can publish a GitHub release from `main`.
