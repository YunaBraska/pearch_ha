...
- `.github/workflows/ci.yml` runs on pull requests, pushes to `main`, and manual dispatch: one job on the latest stable Xcode that first runs `swift run perchha-xcode-doctor --json --strict`, lists XCTest cases with `swift test --disable-swift-testing --enable-xctest list`, then runs `scripts/check.sh` (strict build, tests with the coverage gate, smoke with exported snapshots, repo audit) and fails if the checks dirty the working tree.
- `.github/workflows/release.yml` runs automatically on pushes to `main`, so a real merge to `main` cuts a release. It also supports manual dispatch with an optional `version` override and a branch-safe `dry_run` mode that first verifies native Xcode setup, lists XCTest cases, then builds, packages, verifies, and uploads the release evidence as a workflow artifact without publishing a GitHub release.
